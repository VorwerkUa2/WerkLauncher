...
void TestQuaZipFile::atEnd_data() { bytesAvailable_data(); }
...
void TestQuaZipFile::posRead_data() { bytesAvailable_data(); }
...
void TestQuaZipFile::posWrite_data() { posRead_data(); }
...
void TestQuaZipFile::posWrite() {
  ...
  foreach (QString fileName, fileNames) {
    ...
    for (int i = 0; i < size - size / 2; ++i) {
      zipFile.putChar(
          static_cast<char>(QRandomGenerator::global()->generate()));
    }
...
void TestQuaZipFile::getFileInfo() {
  ...
  QCOMPARE(info32.compressedSize, static_cast<quint32>(info64.compressedSize));
...
void TestQuaZipFile::setFileAttrs() {
  ...
  }
  {
    ...
    quint64 newTime = info.dateTime.toSecsSinceEpoch();
    quint64 oldTime = srcInfo.lastModified().toSecsSinceEpoch();
