...
void TestJlCompress::compressDir_data() {
  ...
  QTest::newRow("simple") << "jldir.zip"
                          ...
                          << (QStringList()
                              ...
                              << "testdir2/subdir/"
                              << "testdir2/subdir/test2sub.txt");
  QTest::newRow("empty dirs")
      << "jldir_empty.zip"
      << (QStringList() << "testdir1/" << "testdir2/testdir3/")
      << (QStringList() << "testdir1/" << "testdir2/"
                        << "testdir2/testdir3/");
  QTest::newRow("hidden files")
      << "jldir_hidden.zip" << (QStringList() << ".test0.txt" << "test1.txt")
...
void TestJlCompress::extractFile_data() {
  ...
  QTest::newRow("russian") << "jlextfilerus.zip"
                           << (QStringList()
                               ...
                               << "testdir2/subdir/test2sub.txt")
                           << QString::fromUtf8("testdir2/тест2.txt")
                           << QString::fromUtf8("тест2.txt")
                           << QByteArray("IBM866");
  QTest::newRow("extract dir")
      << "jlextdir.zip" << (QStringList() << "testdir1/") << "testdir1/"
      << "testdir1/" << QByteArray();
}
...
void TestJlCompress::extractFile() {
  ...
  QVERIFY(!JlCompress::extractFile(zipName, fileToExtract,
               ...
               .isEmpty());
  QFileInfo destInfo("jlext/jlfile/" + destName),
      srcInfo("tmp/" + fileToExtract);
...
void TestJlCompress::extractFiles_data() {
  ...
  QTest::newRow("simple") << "jlextfiles.zip"
                          << (QStringList()
                              ...
                              << "testdir2/subdir/test2sub.txt")
                          << (QStringList()
                              << "testdir2/test2.txt" << "testdir1/test1.txt");
}
...
void TestJlCompress::extractFiles() {
  ...
  QVERIFY(!JlCompress::extractFiles(zipName, filesToExtract, "jlext/jlfiles")
               .isEmpty());
  ...
  QFile zipFile(zipName);
  QVERIFY(zipFile.open(QIODevice::ReadOnly));
  QVERIFY(!JlCompress::extractFiles(&zipFile, filesToExtract, "jlext/jlfiles")
               .isEmpty());
  foreach (QString fileName, filesToExtract) {
    QFileInfo fileInfo("jlext/jlfiles/" + fileName);
    QFileInfo extInfo("tmp/" + fileName);
...
void TestJlCompress::extractDir_data() {
  ...
  QTest::newRow("separate dir")
      << "sepdir.zip" << (QStringList() << "laj/" << "laj/lajfile.txt");
}
...
void TestJlCompress::extractDir() {
  ...
  QCOMPARE((extracted = JlCompress::extractDir(zipName, "jlext/jldir")).count(),
           fileNames.count());
  foreach (QString fileName, fileNames) {
  ...
  QVERIFY(zipFile.open(QIODevice::ReadOnly));
  QCOMPARE(
      (extracted = JlCompress::extractDir(&zipFile, "jlext/jldir")).count(),
      fileNames.count());
  foreach (QString fileName, fileNames) {
    QString fullName = "jlext/jldir/" + fileName;
    QFileInfo fileInfo(fullName);
