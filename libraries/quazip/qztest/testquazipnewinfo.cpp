...
TestQuaZipNewInfo::TestQuaZipNewInfo(QObject *parent) : QObject(parent) {}
      ...
      if (type == 0x000Au && length >= 32) {
        ...
        while (i <= extra.size() - 4) {
          ...
          if (tag == 0x0001u && tagsize >= 24) {
            for (int timesPos = i; i < timesPos + 24; i += 8, tagsize -= 8) {
              quint64 time = static_cast<quint64>(
                             ...
                             (static_cast<quint64>(
                              ...
                              << 8) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 2)))
                              << 16) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 3)))
                              << 24) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 4)))
                              << 32) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 5)))
                              << 40) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 6)))
                              << 48) |
                             (static_cast<quint64>(
                                  static_cast<unsigned char>(extra.at(i + 7)))
                              << 56);
              ++timesFound;
              if (i - timesPos == 0) {
