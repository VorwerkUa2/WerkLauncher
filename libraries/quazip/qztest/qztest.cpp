...
bool createTestFiles(const QStringList &fileNames, int size,
                     const QString &dir) {
  QDir curDir;
  foreach (QString fileName, fileNames) {
    ...
    if (!testDir.exists()) {
      if (!curDir.mkpath(testDir.path())) {
        qWarning("Couldn't mkpath %s", testDir.path().toUtf8().constData());
        return false;
      }
    }
    ...
    if (!fileInfo.isDir()) {
      ...
      while (!file.atEnd()) {
        ...
        if (zipFile.write(buf, l) != l) {
          qWarning("Couldn't write to %s in %s", filePath.toUtf8().constData(),
                   zipName.toUtf8().constData());
          return false;
        }
      }
