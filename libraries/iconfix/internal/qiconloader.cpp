...
/* Theme to use in last resort, if the theme does not have the icon, neither the
 * parents  */

static QString fallbackTheme() { return QString("hicolor"); }
...
static inline QString systemThemeName() { return QIcon::themeName(); }
...
QIconTheme::QIconTheme(const QString &themeName) : m_valid(false) {
  ...
  m_parents =
      indexReader.value(QLatin1String("Icon Theme/Inherits")).toStringList();
...
QIconLoaderEngineFixed::~QIconLoaderEngineFixed() { qDeleteAll(m_entries); }
...
bool QIconLoaderEngineFixed::hasIcon() const { return !(m_entries.isEmpty()); }
...
QPixmap PixmapEntry::pixmap(const QSize &size, QIcon::Mode mode,
  ...
  if (QPixmapCache::find(key, &cachedPixmap)) {
    ...
    if (basePixmap.size() != actualSize) {
      cachedPixmap = basePixmap.scaled(actualSize, Qt::IgnoreAspectRatio,
