...
// byte_array - special case for reading
template <> void tag_array<int8_t>::read_payload(io::stream_reader &reader) {
  int32_t length;
...
// long_array - explicit specialization (must be before generic template)
template <> void tag_array<int64_t>::read_payload(io::stream_reader &reader) {
  int32_t length;
  reader.read_num(length);
  if (length < 0)
    reader.get_istr().setstate(std::ios::failbit);
...
// byte_array - special case for writing
template <>
