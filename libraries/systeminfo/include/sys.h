...
enum class ArchitectureType { Undetermined, X86, AMD64, AARCH64, Unknown };
...
public:
  ...
  bool operator==(const Architecture &rhs) const { return type == rhs.type; }
  bool operator!=(const Architecture &rhs) const { return type != rhs.type; }
  bool operator<(const Architecture &rhs) const { return type < rhs.type; }
  bool operator>(const Architecture &rhs) const { return type > rhs.type; }
  QString getRaw() const { return raw; }
  ...
  static Architecture fromOSArch(const QString &os_arch) {
    ...
    } else if (os_arch == "x86" || os_arch == "i386" || os_arch == "i486" ||
               os_arch == "i586" || os_arch == "i686") {
...
bool lookupSystemStatusCode(uint64_t code, std::string &name,
