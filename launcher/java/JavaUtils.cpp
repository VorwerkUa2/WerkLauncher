...
JavaInstallPtr JavaUtils::MakeJavaPtr(QString path, QString id,
...
QList<JavaInstallPtr> JavaUtils::FindJavaFromRegistryKey(DWORD keyType,
  ...
  HKEY jreKey;
  qDebug() << "Opening registry key:" << keyName << "with type:" << archType;
  if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, (const wchar_t *)keyName.utf16(), 0,
                    KEY_READ | keyType | KEY_ENUMERATE_SUB_KEYS,
                    &jreKey) == ERROR_SUCCESS) {
    // Read the current type version from the registry.
    // This will be used to find any key that contains the JavaHome value.
    WCHAR value[1024];
    DWORD valueSz = sizeof(value) - sizeof(WCHAR);
    if (RegQueryValueExW(jreKey, L"CurrentVersion", NULL, NULL, (BYTE *)value,
                         &valueSz) == ERROR_SUCCESS) {
      value[valueSz / sizeof(WCHAR)] = L'\0';
      // we don't actually use CurrentVersion, but this is how it was...
    }
    ...
    RegQueryInfoKeyW(jreKey, NULL, NULL, NULL, &numSubKeys, NULL, NULL, NULL,
                     NULL, NULL, NULL, NULL);
    qDebug() << "Found" << numSubKeys << "subkeys in" << keyName;

    // Iterate until RegEnumKeyEx fails
    if (numSubKeys > 0) {
      for (DWORD i = 0; i < numSubKeys; i++) {
        subKeyNameSize = 255;
        retCode = RegEnumKeyExW(jreKey, i, subKeyName, &subKeyNameSize, NULL,
                                NULL, NULL, NULL);
        if (retCode == ERROR_SUCCESS) {
          // Now open the registry key for the version that we just got.
          QString subKeyStr =
              QString::fromWCharArray(subKeyName, subKeyNameSize);
          QString newKeyName = keyName + "\\" + subKeyStr + subkeySuffix;
          qDebug() << "  Checking subkey:" << subKeyStr;

          if (RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                            newKeyName.toStdWString().c_str(), 0,
                            KEY_READ | keyType, &newKey) == ERROR_SUCCESS) {
            // Read the JavaHome value to find where Java is installed.
            valueSz = sizeof(value) - sizeof(WCHAR);
            if (RegQueryValueExW(newKey, keyJavaDir.toStdWString().c_str(),
                                 NULL, NULL, (BYTE *)value,
                                 &valueSz) == ERROR_SUCCESS) {
              value[valueSz / sizeof(WCHAR)] = L'\0';
              ...
              javaVersion->path =
                  QDir(FS::PathCombine(QString::fromWCharArray(value), "bin"))
...
QList<QString> JavaUtils::FindJavaPaths() {
  ...
  javas.append("/Applications/Xcode.app/Contents/Applications/Application "
               "Loader.app/Contents/MacOS/itms/java/bin/java");
  javas.append("/Library/Internet "
               "Plug-Ins/JavaAppletPlugin.plugin/Contents/Home/bin/java");
  javas.append("/System/Library/Frameworks/JavaVM.framework/Versions/Current/"
               "Commands/java");
  QDir libraryJVMDir("/Library/Java/JavaVirtualMachines/");
  QStringList libraryJVMJavas =
      libraryJVMDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
  foreach (const QString &java, libraryJVMJavas) {
    javas.append(libraryJVMDir.absolutePath() + "/" + java +
                 "/Contents/Home/bin/java");
    javas.append(libraryJVMDir.absolutePath() + "/" + java +
                 "/Contents/Home/jre/bin/java");
  }
  QDir systemLibraryJVMDir("/System/Library/Java/JavaVirtualMachines/");
  QStringList systemLibraryJVMJavas =
      systemLibraryJVMDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
  foreach (const QString &java, systemLibraryJVMJavas) {
    javas.append(systemLibraryJVMDir.absolutePath() + "/" + java +
                 "/Contents/Home/bin/java");
    javas.append(systemLibraryJVMDir.absolutePath() + "/" + java +
