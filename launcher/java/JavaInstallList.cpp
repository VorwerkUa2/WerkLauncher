...
const BaseVersionPtr JavaInstallList::at(int i) const { return m_vlist.at(i); }

bool JavaInstallList::isLoaded() {
  return m_status == JavaInstallList::Status::Done;
}

int JavaInstallList::count() const { return m_vlist.count(); }
...
void JavaListLoadTask::executeTask() {
  ...
  qDebug() << "Probing" << candidate_paths.size() << "Java paths:";
  int id = 0;
  ...
  for (JavaCheckResult result : results) {
    if (result.validity == JavaCheckResult::Validity::Valid) {
      ...
      qDebug() << " " << javaVersion->id.toString()
               << javaVersion->arch.serialize() << javaVersion->path;
    }
  }
