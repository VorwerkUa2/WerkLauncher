...
#include "minecraft/launch/DownloadAuthlibInjector.h"
...
MinecraftInstance::MinecraftInstance(SettingsObjectPtr globalSettings,
  ...
  m_components->setOldConfigVersion("org.lwjgl",
                                    m_settings->get("LWJGLVersion").toString());
  m_components->setOldConfigVersion("net.minecraftforge",
                                    m_settings->get("ForgeVersion").toString());
  m_components->setOldConfigVersion(
      "com.mumfrey.liteloader",
      m_settings->get("LiteloaderVersion").toString());
}

void MinecraftInstance::saveNow() { m_components->saveNow(); }

QString MinecraftInstance::typeName() const { return "Minecraft"; }
...
QDir MinecraftInstance::jarmodsPath() const { return QDir(jarModsDir()); }
...
QStringList MinecraftInstance::getClassPath() const {
  ...
  Sys::Architecture javaArchitecture =
      Sys::Architecture::deserialize(javaArchitectureStr);
...
QStringList MinecraftInstance::javaArguments() const {
  ...
  // NOTE: Oracle Java option. if there are alternate jvm implementations, this
  // would be the place to customize this for them
#ifdef Q_OS_MAC
...
static QString replaceTokensIn(QString text, QMap<QString, QString> with) {
  ...
  while ((match = token_regexp.match(text, head)).hasMatch()) {
    int start = match.capturedStart();
    result.append(text.mid(tail, start - tail));
    QString key = match.captured(1);
    auto iter = with.find(key);
    if (iter != with.end()) {
      result.append(*iter);
    }
    head = match.capturedEnd();
    tail = head;
  }
...
QStringList MinecraftInstance::processMinecraftArgs(
  ...
  if (quickPlayTarget && !quickPlayTarget->address.isEmpty()) {
    if (m_components->getComponent("net.minecraft")->getReleaseDateTime() >=
        g_VersionFilterData.quickPlayBeginsDate) {
      args_pattern += " --quickPlayMultiplayer " + quickPlayTarget->address +
                      ":" + QString::number(quickPlayTarget->port);
    } else {
  ...
  if (quickPlayTarget && !quickPlayTarget->world.isEmpty()) {
    args_pattern +=
        " --quickPlaySingleplayer \"" + quickPlayTarget->world + "\"";
  }
...
MinecraftInstance::createLaunchScript(AuthSessionPtr session,
  ...
  if (quickPlayTarget && !quickPlayTarget->address.isEmpty()) {
    launchScript +=
        "useQuickPlay " +
        QString::number(
            m_components->getComponent("net.minecraft")->getReleaseDateTime() >=
            g_VersionFilterData.quickPlayBeginsDate) +
        "\n";
    launchScript += "serverAddress " + quickPlayTarget->address + "\n";
    launchScript +=
        "serverPort " + QString::number(quickPlayTarget->port) + "\n";
  }
  ...
  for (auto param : processMinecraftArgs(
           session, nullptr /* When using a launch script, the server and world
...
MessageLevel::Enum MinecraftInstance::guessLevel(const QString &line,
  ...
  // NOTE: this diverges from the real regexp. no unicode, the first section is
  // + instead of *
  ...
  if (m_settings->get("ShowGameTime").toBool()) {
    if (lastTimePlayed() > 0) {
      if (APPLICATION->settings()->get("ShowGameTimeHours").toBool()) {
        description.append(
            tr(", last played for %1 hours")
                .arg(Time::prettifyDurationHours(lastTimePlayed())));
      } else {
        description.append(tr(", last played for %1")
    ...
    if (totalTimePlayed() > 0) {
      if (APPLICATION->settings()->get("ShowGameTimeHours").toBool()) {
        description.append(
            tr(", total played for %1 hours")
                .arg(Time::prettifyDurationHours(totalTimePlayed())));
      } else {
        description.append(tr(", total played for %1")
...
MinecraftInstance::createLaunchTask(AuthSessionPtr session,
  ...
  if (!quickPlayTarget && m_settings->get("JoinWorldOnLaunch").toBool()) {
    if (m_settings->get("JoinServerOnLaunch").toBool()) {
      ...
      quickPlayTarget.reset(
          new QuickPlayTarget(QuickPlayTarget::parseMultiplayer(fullAddress)));
    } else if (m_settings->get("JoinSingleplayerWorldOnLaunch").toBool()) {
      QString worldName =
          m_settings->get("JoinSingleplayerWorldOnLaunchName").toString();
      quickPlayTarget.reset(
          new QuickPlayTarget(QuickPlayTarget::parseSingleplayer(worldName)));
    }
  }
  ...
  if (session && !session->sync_url.isEmpty()) {
    process->appendStep(new DownloadAuthlibInjector(pptr, session));
  }
...
std::shared_ptr<ModFolderModel> MinecraftInstance::loaderModList() const {
  if (!m_loader_mod_list) {
    ...
    connect(this, &BaseInstance::runningStatusChanged, m_loader_mod_list.get(),
...
std::shared_ptr<ModFolderModel> MinecraftInstance::coreModList() const {
  if (!m_core_mod_list) {
    ...
    connect(this, &BaseInstance::runningStatusChanged, m_core_mod_list.get(),
            &ModFolderModel::disableInteraction);
  }
...
std::shared_ptr<ModFolderModel> MinecraftInstance::resourcePackList() const {
  if (!m_resource_pack_list) {
    ...
    connect(this, &BaseInstance::runningStatusChanged,
            m_resource_pack_list.get(), &ModFolderModel::disableInteraction);
  }
...
std::shared_ptr<ModFolderModel> MinecraftInstance::shaderPackList() const {
  if (!m_shader_pack_list) {
    ...
    connect(this, &BaseInstance::runningStatusChanged, m_shader_pack_list.get(),
            &ModFolderModel::disableInteraction);
  }
