...
void WorldList::directoryChanged(QString path) { update(); }

bool WorldList::isValid() { return m_dir.exists() && m_dir.isReadable(); }
...
int WorldList::columnCount(const QModelIndex &parent) const { return 3; }
...
QVariant WorldList::data(const QModelIndex &index, int role) const {
  ...
  case FolderRole: {
    ...
    return QVariant::fromValue(world.seed());
  case NameRole: {
    return world.name();
  }
  case LastPlayedRole: {
