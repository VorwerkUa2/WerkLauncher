...
PackProfile::~PackProfile() { saveNow(); }
...
static ComponentPtr componentFromJsonV1(PackProfile *parent,
  ...
  component->m_dependencyOnly =
      Json::ensureValueBoolean(obj.value("dependencyOnly"), false);
  component->m_important =
      Json::ensureValueBoolean(obj.value("important"), false);

  // cached
  // TODO @RESILIENCE: ignore invalid values/structure here?
  component->m_cachedVersion =
      Json::ensureValueString(obj.value("cachedVersion"));
  ...
  component->m_cachedVolatile =
      Json::ensureValueBoolean(obj.value("volatile"), false);
...
bool PackProfile::saveIsScheduled() const { return d->dirty; }
...
void PackProfile::scheduleSave() {
  if (!d->loaded) {
    qDebug() << "Component list should never save if it didn't successfully "
                "load, instance:"
             << d->m_instance->name();
    return;
  }
...
bool PackProfile::load() {
  ...
  if (!componentsFile.exists()) {
    if (!migratePreComponentConfig()) {
      ...
      qCritical() << "Failed to convert old pre-component config for instance"
                  << d->m_instance->name();
      return false;
    }
  }
...
Task::Ptr PackProfile::getCurrentTask() { return d->m_updateTask; }
...
bool PackProfile::migratePreComponentConfig() {
  ...
  auto addBuiltinPatch = [&](const QString &uid, bool asDependency,
                             const QString &emptyVersion,
                             const Meta::Require &req,
                             const Meta::Require &conflict) {
  ...
  ProfileUtils::readOverrideOrders(
      FS::PathCombine(d->m_instance->instanceRoot(), "order.json"), userOrder);
  ...
  if (!loadedComponents.isEmpty()) {
    ...
    for (auto order : files.keys()) {
      ...
      for (auto &value : values) {
        // TODO: put back the insertion of problem messages here, so the user
        // knows about the id duplication
...
void PackProfile::insertComponent(size_t index, ComponentPtr component) {
  ...
  beginInsertRows(QModelIndex(), static_cast<int>(index),
...
void PackProfile::invalidateLaunchProfile() { d->m_profile.reset(); }
  ...
  auto preRemoveJarMod = [&](LibraryPtr jarMod) -> bool {
    ...
    if (finfo.exists()) {
      ...
      if (!jarModFile.remove()) {
        qCritical() << "File" << jar[0] << "could not be removed because:"
    ...
    try {
      ...
      for (auto file : d->components) {
        qDebug() << "Applying" << file->getID()
                 << (file->getProblemSeverity() == ProblemSeverity::Error
                         ? "ERROR"
                         : "GOOD");
