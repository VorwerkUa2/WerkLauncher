...
bool parseXTokenResponse(QByteArray &data, Katabasis::Token &output,
  ...
  if (jsonError.error) {
    qWarning()
        << "Failed to parse response from user.auth.xboxlive.com as JSON: "
...
bool parseForcedMigrationResponse(QByteArray &data, bool &result) {
  ...
  QJsonDocument doc = QJsonDocument::fromJson(data, &jsonError);
  if (jsonError.error) {
    qWarning() << "Failed to parse response from " << BuildConfig.API_BASE
               << "/rollout/v1/msamigrationforced as JSON: "
               << jsonError.errorString();
    return false;
  }
  ...
  if (feature != "msamigrationforced") {
    qWarning() << "Rollout feature is not what we expected "
                  "(msamigrationforced), but is instead \""
               << feature << "\"";
    return false;
  }
...
bool parseRolloutResponse(QByteArray &data, bool &result) {
  ...
  if (jsonError.error) {
    qWarning() << "Failed to parse response from " << BuildConfig.API_BASE
               << "/rollout/v1/msamigration as JSON: "
               << jsonError.errorString();
    return false;
  }
  ...
  if (feature != "msamigration") {
    qWarning() << "Rollout feature is not what we expected (msamigration), but "
                  "is instead \""
               << feature << "\"";
    return false;
  }
...
bool parseMojangResponse(QByteArray &data, Katabasis::Token &output) {
  ...
  if (jsonError.error) {
    qWarning() << "Failed to parse response from " << BuildConfig.API_BASE
               << "/launcher/login as JSON: " << jsonError.errorString();
    return false;
  }
