...
QString AccountTask::getStateMessage() const {
  ...
  case AccountTaskState::STATE_FAILED_MUST_MIGRATE:
    return tr("Failed to authenticate. The account must be migrated to a "
              "Microsoft account to be usable.");
...
MojangError MojangError::fromJSON(QByteArray data,
...
QString MojangError::toString() const {
  ...
  if (jsonParsed) {
    ...
    out << "Mojang error failed to parse with error: "
