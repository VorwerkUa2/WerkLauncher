...
Katabasis::Token tokenFromJSONV3(const QJsonObject &parent,
  ...
  if (notAfter.isDouble()) {
    out.notAfter =
        QDateTime::fromMSecsSinceEpoch(((int64_t)notAfter.toDouble()) * 1000);
  }
...
MinecraftProfile profileFromJSONV3(const QJsonObject &parent,
  ...
  }
  {
    ...
    if (!idV.isString() || !nameV.isString()) {
      qWarning()
          << "mandatory profile attributes are missing or of unexpected type";
...

  {
    ...
    if (!idV.isString() || !urlV.isString() || !variantV.isString()) {
      qWarning()
          << "mandatory skin attributes are missing or of unexpected type";
      return MinecraftProfile();
    }
...
bool AccountData::resumeStateFromV3(QJsonObject data) {
  ...
  syncUrl = data.value("sync_url").toString();
  syncUsername = data.value("sync_user").toString();
  syncPassword = data.value("sync_pass").toString();
  ...
  if (type == AccountType::Local) {
    minecraftEntitlement.ownsMinecraft = true;
    minecraftEntitlement.canPlayMinecraft = true;
    minecraftEntitlement.validity = Katabasis::Validity::Certain;
...
QJsonObject AccountData::saveState() const {
  ...
  if (!syncUrl.isEmpty()) {
    output["sync_url"] = syncUrl;
    output["sync_user"] = syncUsername;
    output["sync_pass"] = syncPassword;
  }
...
QString AccountData::accessToken() const { return yggdrasilToken.token; }

QString AccountData::profileId() const { return minecraftProfile.id; }

QString AccountData::xid() const {
  if (type == AccountType::LittleSkin || type == AccountType::Local) {
    // Generate a unique, stable pseudo-XID for custom accounts to prevent
    // overwriting in AccountList Use a combination of sync URL (hashed) and
    // profile ID (nickname-based)
    QString salt =
        QCryptographicHash::hash(syncUrl.toUtf8(), QCryptographicHash::Sha1)
            .toHex()
            .left(8);
    return QString("custom:%1:%2").arg(salt, minecraftProfile.id);
  }
  return xboxApiToken.extra["xid"].toString();
}

QString AccountData::profileName() const { return minecraftProfile.name; }

QString AccountData::gamerTag() const {
  return xboxApiToken.extra["gtg"].toString();
}

QString AccountData::lastError() const { return errorString; }
