...
public:
  ...
  bool getAccountByXID(const QString &xid, MinecraftAccountPtr &pointer,
                       int &index) const;
  bool getAccountByProfileName(const QString &profileName,
                               MinecraftAccountPtr &pointer, int &index) const;
  bool getAccountById(const QString &internalId, MinecraftAccountPtr &pointer,
                      int &index) const;
...
public slots:
  ...
  /**
   * This is called when a (refresh/login) task involving the account starts or
   * ends
   */
...
protected:
   ...
   * This emits the listChanged() signal and autosaves the list (if autosave is
   * enabled).
   */
  void onListChanged();
  ...
  /*!
   * If true, the account list will automatically save to the account list path
   * when it changes. Ignored if m_listFilePath is blank.
   */
