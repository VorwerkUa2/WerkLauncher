...
void XboxAuthorizationStep::onRequestDone(
  ...
  if (error != QNetworkReply::NoError) {
    ...
    if (!processSTSError(error, data, headers)) {
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("Failed to get authorization for %1 services. Error %2.")
  ...
  if (!Parsers::parseXTokenResponse(data, temp, m_authorizationKind)) {
    emit finished(
        ...
        tr("Could not parse authorization response for access to %1 services.")
            .arg(m_authorizationKind));
    return;
  }
...
bool XboxAuthorizationStep::processSTSError(
  ...
  if (error == QNetworkReply::AuthenticationRequiredError) {
    ...
    if (!Parsers::getNumber(obj.value("XErr"), errorCode)) {
      emit finished(
          AccountTaskState::STATE_FAILED_SOFT,
          tr("XErr element is missing from %1 authorization error response.")
              .arg(m_authorizationKind));
      return true;
    }
    switch (errorCode) {
    case 2148916227: {
      // NOTE: this is the error experienced by a number of people on Discord
      // using dodgy alt accounts
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("Your XBox Live account has been banned by Microsoft "
                       "for violating the XBox Community Standards.\nThis may "
                       "happen if your account was shared or resold."));
      return true;
    }
    case 2148916229: {
      emit finished(
          AccountTaskState::STATE_FAILED_SOFT,
          tr("This Microsoft account is linked to a family and your parent or "
             "guardian has not given you permission to play online."));
      return true;
    }
    case 2148916233: {
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
    ...
    case 2148916234: {
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("This account has not accepted the XBox Terms of "
                       "Service. Please log in online and accept them."));
      return true;
    }
    case 2148916235: {
      // NOTE: this is the Grulovia error
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("XBox Live is not available in your country. You've "
                       "been blocked."));
      return true;
    }
    case 2148916236: {
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("This Microsoft account requires proof of age to play. "
                       "Please login to %1 to provide proof of age.")
                        .arg("<a "
                             "href=\"https://login.live.com/"
                             "login.srf\">login.live.com</a>"));
      return true;
    }
    case 2148916237: {
      emit finished(AccountTaskState::STATE_FAILED_SOFT,
                    tr("This Microsoft account has reached its playtime limit "
                       "and has been blocked from logging in."));
      return true;
    }
    case 2148916238: {
      emit finished(
          AccountTaskState::STATE_FAILED_SOFT,
          tr("This Microsoft account is underaged and is not linked to a "
             "family.\n\nPlease set up your account according to %1.")
              .arg("<a "
                   "href=\"https://help.minecraft.net/hc/en-us/articles/"
                   "4408968616077\">help.minecraft.net</a>"));
      return true;
    }
    default: {
      emit finished(
