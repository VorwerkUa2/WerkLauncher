...
bool AccountList::getAccountByXID(const QString &xid,
                                  MinecraftAccountPtr &pointer,
                                  int &index) const {
...
bool AccountList::getAccountByProfileName(const QString &profileName,
                                          MinecraftAccountPtr &pointer,
                                          int &index) const {
  for (int i = 0; i < count(); i++) {
    auto entry = at(i);
...
bool AccountList::getAccountById(const QString &internalId,
                                 MinecraftAccountPtr &pointer,
                                 int &index) const {
  for (int i = 0; i < count(); i++) {
    auto entry = at(i);
...
QModelIndex AccountList::addAccount(const MinecraftAccountPtr account) {
  ...
  connect(account.get(), &MinecraftAccount::changed, this,
          &AccountList::onAccountChanged);
  connect(account.get(), &MinecraftAccount::activityChanged, this,
          &AccountList::onAccountActivityChanged);
...
void AccountList::setDefaultAccount(MinecraftAccountPtr newAccount) {
  if (!newAccount && m_defaultAccount) {
    ...
    if (currentDefaultAccount != newDefaultAccount) {
      ...
      emit dataChanged(index(newDefaultAccountIdx),
...
int AccountList::count() const { return m_accounts.count(); }

QVariant AccountList::data(const QModelIndex &index, int role) const {
  qCDebug(L_ACCOUNTS) << "AccountList::data row:" << index.row()
                      << "role:" << role << "count:" << count();
  if (index.row() < 0 || index.row() >= rowCount(QModelIndex())) {
    qCCritical(L_ACCOUNTS) << "AccountList::data called with out of bounds row:"
                           << index.row()
                           << "rowCount:" << rowCount(QModelIndex());
    return QVariant();
  }

  if (index.row() >= m_accounts.count()) {
    qCDebug(L_ACCOUNTS) << "AccountList::data accessing special row (Add New)";
    switch (role) {
    case Qt::DisplayRole:
    ...
    case PointerRole:
      qCDebug(L_ACCOUNTS)
          << "Returning empty MinecraftAccountPtr for special row";
  ...
  qCDebug(L_ACCOUNTS) << "AccountList::data entry isAccount:"
                      << entry.isAccount;
  if (!entry.isAccount) {
    // This part should technically not be reachable if all entries are accounts
    switch (role) {
    case Qt::DisplayRole:
      return tr("Add New Account");
    case Qt::DecorationRole:
  ...
  switch (role) {
  case Qt::DisplayRole: {
    return account->profileName() + " (" + account->gamerTag() + ")\n" +
...
bool AccountList::loadList() {
  if (m_listFilePath.isEmpty()) {
    qCritical()
        << "Can't load account list. No file path given and no default set.";
    return false;
  }

  m_accounts.clear();
  m_accounts.append(Entry{false, MinecraftAccountPtr()});
  ...
  default: {
    ...
    qWarning() << "Unknown format version when loading account list. Existing "
                  "one will be renamed to"
               << newName;
...
bool AccountList::saveList() {
  if (m_listFilePath.isEmpty()) {
    qCritical()
        << "Can't save account list. No file path given and no default set.";
    return false;
  }
...
bool AccountList::anyAccount() const {
  for (auto &entry : m_accounts) {
    if (entry.isAccount) {
      return true;
    }
      ...
      if (entry.account->internalId() == accountId) {
        ...
        if (m_currentTask) {
          ...
          qDebug() << "RefreshSchedule: Processing account "
                   << entry.account->gamerTag() << " with internal ID "
                   << accountId;
          return;
        }
      }
    }
    qDebug() << "RefreshSchedule: Account with with internal ID " << accountId
             << " not found.";
  }
...
bool AccountList::isActive() const { return m_activityCount != 0; }

void AccountList::beginActivity() { m_activityCount++; }
