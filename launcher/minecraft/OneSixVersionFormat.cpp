...
VersionFilePtr
OneSixVersionFormat::versionFileFromJson(const QJsonDocument &doc,
                                         const QString &filename,
  ...
  } else if (root.contains("+jarMods")) // DEPRECATED: old style '+jarMods' are
                                        // only here for backwards compatibility
  {
    for (auto libVal : requireValueArray(root.value("+jarMods"))) {
      QJsonObject libObj = requireValueObject(libVal);
      // parse the jarmod
      auto lib = OneSixVersionFormat::plusJarModFromJson(*out, libObj, filename,
                                                         out->name);
      // and add to jar mods
      out->jarMods.append(lib);
    }
  ...
  if (hasPlusLibs && hasLibs) {
    out->addProblem(ProblemSeverity::Warning,
                    QObject::tr("Version file has both '+libraries' and "
                                "'libraries'. This is no longer supported."));
  ...
  // else reconstruct it from downloads and id ... if that's available
    ...
    lib->setRawName(GradleSpecifier(
        QString("com.mojang:minecraft:%1:client").arg(out->minecraftVersion)));
    ...
    // we got nothing...
      ...
      out->addProblem(
          ...
          QObject::tr("URL for the main jar could not be determined - Mojang "
  ...
  if (root.contains("tweakers")) {
    out->addProblem(
        ProblemSeverity::Error,
        QObject::tr("Version file contains unsupported element 'tweakers'"));
  }
  if (root.contains("-libraries")) {
    out->addProblem(
        ProblemSeverity::Error,
        QObject::tr("Version file contains unsupported element '-libraries'"));
  }
  if (root.contains("-tweakers")) {
    out->addProblem(
        ProblemSeverity::Error,
  ...
  if (root.contains("-minecraftArguments")) {
    out->addProblem(
        ProblemSeverity::Error,
        QObject::tr(
            "Version file contains unsupported element '-minecraftArguments'"));
  }
  if (root.contains("+minecraftArguments")) {
    out->addProblem(
        ProblemSeverity::Error,
...
LibraryPtr OneSixVersionFormat::plusJarModFromJson(
    ProblemContainer &problems, const QJsonObject &libObj,
    const QString &filename, const QString &originalName) {
