...
AssetUpdateTask::AssetUpdateTask(MinecraftInstance *inst) { m_inst = inst; }
...
void AssetUpdateTask::executeTask() {
  ...
  auto dl = Net::Download::makeCached(indexUrl, entry,
                                      Net::Download::Option::AcceptLocalFiles);
...
bool AssetUpdateTask::canAbort() const { return true; }
...
void AssetUpdateTask::assetIndexFailed(QString reason) {
  qDebug() << m_inst->name()
           << ": Failed asset index download. Checking for cache...";
  auto components = m_inst->getPackProfile();
  auto profile = components->getProfile();
  auto assets = profile->getMinecraftAssets();
  QString asset_fname = "assets/indexes/" + assets->id + ".json";
  ...
  if (QFile::exists(asset_fname)) {
    qDebug() << m_inst->name()
             << ": Cache hit for asset index, proceeding offline.";
    assetIndexFinished();
  } else {
