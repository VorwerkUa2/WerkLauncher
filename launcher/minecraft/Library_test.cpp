...
private slots:
  ...
  void test_legacy_url() {
    ...
    auto downloads =
        test.getDownloads(currentSystem, cache.get(), failedFiles, QString());
    QCOMPARE(downloads.size(), 1);
    QCOMPARE(failedFiles, {});
    NetAction::Ptr dl = downloads[0];
    QCOMPARE(dl->m_url, QUrl("file://foo/bar/test/package/testname/testversion/"
                             "testname-testversion.jar"));
  }
  ...
  void test_legacy_native() {
    ...
    test.setRepositoryURL("file://foo/bar");
    {
      ...
      QStringList failedFiles;
      auto dls =
          test.getDownloads(Os_Linux, cache.get(), failedFiles, QString());
      QCOMPARE(dls.size(), 1);
      QCOMPARE(failedFiles, {});
      auto dl = dls[0];
      QCOMPARE(dl->m_url, QUrl("file://foo/bar/test/package/testname/"
                               "testversion/testname-testversion-linux.jar"));
    }
  }
  ...
  void test_legacy_native_arch_local_override() {
    ...
    test.setRepositoryURL("file://foo/bar");
    {
      ...
      QCOMPARE(jar, {});
      QCOMPARE(native, {});
      QCOMPARE(native32, {QFileInfo("data/testname-testversion-linux-32.jar")
                              .absoluteFilePath()});
      QCOMPARE(native64, {QFileInfo("data/testname-testversion-linux-64.jar")
                              .absoluteFilePath()});
      QStringList failedFiles;
  ...
  void test_onenine() {
    auto test = readMojangJson("data/lib-simple.json");
    {
      ...
      QCOMPARE(
          jar,
          getStorage("com/paulscode/codecwav/20101023/codecwav-20101023.jar"));
      QCOMPARE(native, {});
      QCOMPARE(native32, {});
      QCOMPARE(native64, {});
    }
    {
      ...
      QCOMPARE(failedFiles, {});
      QCOMPARE(dls[0]->m_url,
               QUrl("https://libraries.minecraft.net/com/paulscode/codecwav/"
                    "20101023/codecwav-20101023.jar"));
    }
    test->setHint("local");
    {
      QStringList jar, native, native32, native64;
  ...
  void test_onenine_native() {
    ...
    QCOMPARE(
        native,
        getStorage("org/lwjgl/lwjgl/lwjgl-platform/2.9.4-nightly-20150209/"
