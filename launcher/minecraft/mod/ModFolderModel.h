...
public:
  ...
  virtual int rowCount(const QModelIndex &) const override { return size(); }

  virtual QVariant headerData(int section, Qt::Orientation orientation,
                              int role = Qt::DisplayRole) const override;
  virtual int columnCount(const QModelIndex &parent) const override;

  int size() const { return static_cast<int>(mods.size()); };
  bool empty() const { return size() == 0; }
  Mod &operator[](size_t index) { return mods[static_cast<int>(index)]; }
  const Mod &at(size_t index) const { return mods.at(static_cast<int>(index)); }
  ...
  const QList<Mod> &allMods() { return mods; }
