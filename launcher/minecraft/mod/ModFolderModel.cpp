...
void ModFolderModel::finishUpdate() {
  QSet<QString> currentSet(modsIndex.keyBegin(), modsIndex.keyEnd());
  auto &newMods = m_update->mods;
  QSet<QString> newSet(newMods.keyBegin(), newMods.keyEnd());
...
void ModFolderModel::directoryChanged(QString path) { update(); }

bool ModFolderModel::isValid() { return m_dir.exists() && m_dir.isReadable(); }
...
bool ModFolderModel::installMod(const QString &filename) {
  ...
  // NOTE: fix for GH-1178: remove trailing slash to avoid issues with using the
  // empty result of QFileInfo::fileName
  auto originalPath = FS::NormalizePath(filename);
  QFileInfo fileinfo(originalPath);

  if (!fileinfo.exists() || !fileinfo.isReadable()) {
    qWarning()
        << "Caught attempt to install non-existing file or file-like object:"
        << originalPath;
    return false;
  }
  ...
  if (type == Mod::MOD_SINGLEFILE || type == Mod::MOD_ZIPFILE ||
    ...
    if (!FS::copy(from, newpath)()) {
      qWarning() << "Copy of folder from" << originalPath << "to" << newpath
...
bool ModFolderModel::setModStatus(int row,
  ...
  if (modsIndex.contains(newId)) {
    // NOTE: this could handle a corner case, where we are overwriting a file,
    // because the same 'mod' exists both enabled and disabled But is it
    // necessary?
  }
