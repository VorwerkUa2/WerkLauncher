...
static VersionFilePtr createErrorVersionFile(QString fileId, QString filepath,
                                             QString error) {
...
VersionFilePtr parseJsonFile(const QFileInfo &fileInfo,
  ...
  if (!file.open(QFile::ReadOnly)) {
    auto errorStr = QObject::tr("Unable to open the version file %1: %2.")
                        .arg(fileInfo.fileName(), file.errorString());
    return createErrorVersionFile(fileInfo.completeBaseName(),
                                  fileInfo.absoluteFilePath(), errorStr);
  }
  QJsonParseError error;
  ...
  if (error.error != QJsonParseError::NoError) {
    ...
    auto errorStr =
        QObject::tr(
            "Unable to process the version file %1: %2 at line %3 column %4.")
