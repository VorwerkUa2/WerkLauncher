...
static LoadResult loadComponent(ComponentPtr component, Task::Ptr &loadTask,
  ...
  if (QFile::exists(customPatchFilename)) {
    ...
    auto file =
        ProfileUtils::parseJsonFile(QFileInfo(customPatchFilename), false);
    ...
    auto metaVersion = APPLICATION->metadataIndex()->get(component->m_uid,
                                                         component->m_version);
...
/*
static LoadResult loadPackProfile(ComponentPtr component, Task::Ptr& loadTask,
Net::Mode netmode)
{
    if(component->m_loaded)
    {
...
void ComponentUpdateTask::loadComponents() {
  ...
  for (auto component : d->m_list->d->components) {
    ...
    // FIXME: to do this right, we need to load the lists and decide on which
    // versions to use during dependency resolution. For now, ignore all that...
#if 0
        switch(d->mode)
        {
            case Mode::Launch:
    ...
    for (const auto &componentRequire : componentRequires) {
      ...
      if (found != output.cend()) {
        ...
        if (result.ok) {
          ...
          qCritical() << "Conflicting requirements:" << componentRequire.uid
...
void ComponentUpdateTask::resolveDependencies(bool checkOnly) {
   ...
   * 1. There are conflicting dependencies on the same uid with different exact
   * version numbers
   *    -> hard error
  ...
  do {
    ...
    if (!gatherRequirementsFromComponents(components, allRequires)) {
      emitFailed(
          tr("Conflicting requirements detected during dependency checking!"));
      return;
    }
  ...
  RequireExSet toChange;
  bool succeeded =
      getTrivialComponentChanges(componentIndex, allRequires, toAdd, toChange);
  if (!succeeded) {
  ...
  if (toAdd.size()) {
    ...
    for (auto &add : toAdd) {
      ...
      if (!add.equalsVersion.isEmpty()) {
        // exact version
        qDebug() << "Adding" << add.uid << "version" << add.equalsVersion
        ...
        // ############################################################################################################
        // HACK HACK HACK HACK FIXME: this is a placeholder for deciding what
        // version to use. For now, it is hardcoded.
        if (!add.suggests.isEmpty()) {
          ...
          if (add.uid == "org.lwjgl") {
            ...
            component->m_version = "3.1.2";
                     ...
                     add.uid == "org.quiltmc.hashed") {
            auto minecraft = std::find_if(
                components.begin(), components.end(), [](ComponentPtr &cmp) {
...
void ComponentUpdateTask::remoteLoadSucceeded(size_t taskIndex) {
  ...
  if (taskSlot.type == RemoteLoadStatus::Type::Version) {
    auto component =
        d->m_list->getComponent(static_cast<int>(taskSlot.PackProfileIndex));
    component->m_loaded = true;
    component->updateCachedData();
  }
  checkIfAllFinished();
...
void ComponentUpdateTask::remoteLoadFailed(size_t taskIndex,
                                           const QString &msg) {
  auto &taskSlot = d->remoteLoadStatusList[static_cast<int>(taskIndex)];
  if (taskSlot.finished) {
    qWarning() << "Got multiple results from remote load task" << taskIndex;
