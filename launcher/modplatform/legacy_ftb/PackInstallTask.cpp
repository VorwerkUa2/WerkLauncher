...
void PackInstallTask::executeTask() { downloadPack(); }
...
void PackInstallTask::downloadPack() {
  ...
  if (m_pack.type == PackType::Private) {
    url = QString(BuildConfig.LEGACY_FTB_CDN_BASE_URL + "privatepacks/%1")
              .arg(packoffset);
  } else {
...
void PackInstallTask::onDownloadProgress(qint64 current, qint64 total) {
  ...
  setStatus(
      tr("Downloading zip for %1 (%2%)").arg(m_pack.name).arg(current / 10));
}
  ...
  m_extractFuture =
      QtConcurrent::run(QThreadPool::globalInstance(),
                        [archivePath = archivePath,
                         unzipPath = extractDir.absolutePath() + "/unzip"] {
...
void PackInstallTask::onUnzipFinished() { install(); }

void PackInstallTask::onUnzipCanceled() { emitAborted(); }
...
void PackInstallTask::install() {
  ...
  if (packJson.exists()) {
    (void)packJson.open(QIODevice::ReadOnly | QIODevice::Text);
