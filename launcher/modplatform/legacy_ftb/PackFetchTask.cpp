...
void PackFetchTask::fetch() {
  ...
  jobPtr->addNetAction(
      Net::Download::makeByteArray(publicPacksUrl, &publicModpacksXmlFileData));
...
void PackFetchTask::fetchPrivate(const QStringList &toFetch) {
  ...
  for (auto &packCode : toFetch) {
    ...
    job->addNetAction(
        Net::Download::makeByteArray(privatePackBaseUrl.arg(packCode), data));
...
void PackFetchTask::fileDownloadFinished() {
  ...
  if (!parseAndAddPacks(thirdPartyModpacksXmlFileData, PackType::ThirdParty,
                        thirdPartyPacks)) {
  ...
  for (int i = 0; i < nodes.length(); i++) {
    ...
    if (modpack.oldVersions.size() < 1) {
      if (!modpack.currentVersion.isNull() &&
        ...
        qWarning() << "Added current version to oldVersions because "
                      "oldVersions was empty! (" +
                          modpack.name + ")";
      } else {
        modpack.broken = true;
