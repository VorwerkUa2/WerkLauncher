...
PackInstallTask::PackInstallTask(UserInteractionSupport *support,
                                 QString packName, QString version) {
  m_support = support;
...
void PackInstallTask::onDownloadSucceeded() {
  ...
  if (!vlist) {
    emitFailed(
        tr("Failed to get local metadata index for %1").arg("net.minecraft"));
    return;
  }
  ...
  if (m_version.loader.recommended || m_version.loader.latest ||
    ...
    if (m_version.loader.recommended || m_version.loader.latest) {
      for (int i = 0; i < vlist->versions().size(); i++) {
        ...
        // version. not all mod loaders depend on a given Minecraft version, so
        // we won't do this filtering for those loaders.
  ...
  for (const auto &lib : m_version.libraries) {
    ...
    for (const auto &existingLib : exempt) {
      if (libSpecifier.matchName(existingLib)) {
        ...
        libExempt =
            Version(libSpecifier.version()) >= Version(existingLib.version());
      }
    }
    if (libExempt)
      continue;
    ...
    case DownloadType::Unknown:
      emitFailed(
          tr("Unknown or unsupported download type: %1").arg(lib.download_raw));
      return false;
    }
...
bool PackInstallTask::createPackComponent(
    QString instanceRoot, std::shared_ptr<PackProfile> profile) {
  if (m_version.mainClass.mainClass.isEmpty() &&
      m_version.extraArguments.arguments.isEmpty()) {
    return true;
  }
  ...
  if (hasMainClassDepends || hasExtraArgumentsDepends) {
    ...
    if (hasExtraArgumentsDepends &&
        !mods.contains(m_version.extraArguments.depends)) {
      extraArguments = "";
    }
  }
...
void PackInstallTask::installConfigs() {
  ...
  auto path =
      QString("Configs/%1/%2.zip").arg(m_pack_safe_name).arg(m_version_name);
  ...
  m_extractFuture =
      QtConcurrent::run(QThreadPool::globalInstance(),
                        [archivePath = archivePath,
                         unzipPath = extractDir.absolutePath() + "/minecraft"] {
                          return MMCZip::extractDir(archivePath, unzipPath);
                        });
...
void PackInstallTask::downloadMods() {
  ...
  for (const auto &mod : m_version.mods) {
    ...
    auto cacheName =
        fileName.completeBaseName() + "-" + mod.md5 + "." + fileName.suffix();

    if (mod.type == ModType::Extract ||
        mod.type == ModType::TexturePackExtract ||
        mod.type == ModType::ResourcePackExtract) {
      auto entry =
          APPLICATION->metacache()->resolveEntry("ATLauncherPacks", cacheName);
      entry->setStale(true);
...
void PackInstallTask::onModsDownloaded() {
  ...
  if (!modsToExtract.empty() || !modsToDecomp.empty() || !modsToCopy.empty()) {
    m_modExtractFuture = QtConcurrent::run(
        ...
        [this, modsToExtract = modsToExtract, modsToDecomp = modsToDecomp,
         modsToCopy = modsToCopy] {
          return extractMods(modsToExtract, modsToDecomp, modsToCopy);
        });
    connect(&m_modExtractFutureWatcher, &QFutureWatcher<QStringList>::finished,
            this, &PackInstallTask::onModsExtracted);
    connect(&m_modExtractFutureWatcher, &QFutureWatcher<QStringList>::canceled,
            this, [&]() { emitAborted(); });
...
bool PackInstallTask::extractMods(const QMap<QString, VersionMod> &toExtract,
  ...
  for (auto iter = toExtract.begin(); iter != toExtract.end(); iter++) {
    ...
    QDir extractDir(m_stagingPath);
    auto extractToPath =
        FS::PathCombine(extractDir.absolutePath(), "minecraft", extractToDir);
  ...
  for (auto iter = toDecomp.begin(); iter != toDecomp.end(); iter++) {
    ...
    QDir extractDir(m_stagingPath);
    auto extractToPath = FS::PathCombine(extractDir.absolutePath(), "minecraft",
  ...
  for (auto iter = toCopy.begin(); iter != toCopy.end(); iter++) {
    ...
    // If the file already exists, assume the mod is the correct copy - and
    // remove the copy from the Configs.zip
...
void PackInstallTask::install() {
  ...
  instance.setManagedPack("atlauncher", m_pack_safe_name, m_pack_name,
                          m_version_name, m_version_name);
