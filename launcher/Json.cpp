...
QByteArray toBinary(const QJsonObject &obj) {
  return QCborValue::fromVariant(QJsonDocument(obj).toVariant()).toCbor();
}
QByteArray toBinary(const QJsonArray &array) {
  return QCborValue::fromVariant(QJsonDocument(array).toVariant()).toCbor();
}
...
static bool isBinaryJson(const QByteArray &data) {
  // QCborValue::fromBinaryData will return a null document if it's not CBOR,
  // but CBOR doesn't have a simple fixed tag like the old binary JSON.
  // However, we can check if it starts with things that are NOT '{' or '[' if
...
QJsonDocument requireDocument(const QByteArray &data, const QString &what) {
  if (isBinaryJson(data)) {
    QJsonDocument doc =
        QJsonDocument::fromVariant(QCborValue::fromCbor(data).toVariant());
    if (doc.isNull()) {
      throw JsonException(what + ": Invalid JSON (binary JSON detected)");
    }
    return doc;
  } else if (data.startsWith("\xbf")) { // CBOR map start
    return QJsonDocument::fromVariant(QCborValue::fromCbor(data).toVariant());
  } else {
