...
static void RenderQuad(RenderContext &context, const QVector3D &p0,
                       const QVector3D &p1, const QVector3D &p2,
                       const QVector3D &p3, const Rectangle &mapping,
                       const Texture texture, const bool transparency,
                       const float wPixel, const float hPixel) {
  ...
  context.m_vertexBuffer.push_back(
      Vertex(p0, left, bottom, textureId, transparency));
  context.m_vertexBuffer.push_back(
      Vertex(p1, right, bottom, textureId, transparency));
  context.m_vertexBuffer.push_back(
      Vertex(p2, right, top, textureId, transparency));
  context.m_vertexBuffer.push_back(
      Vertex(p3, left, top, textureId, transparency));
...
void RenderBox(RenderContext &context, float width, float height, float depth,
               const TextureMapping &mapping, QMatrix4x4 transform) {
  ...
  RenderQuad(context, vertices[0], vertices[1], vertices[2], vertices[3],
             mapping.front, mapping.material, mapping.transparent, wPixel,
             hPixel);

  RenderQuad(context, vertices[1], vertices[5], vertices[6], vertices[2],
             mapping.left, mapping.material, mapping.transparent, wPixel,
             hPixel);

  RenderQuad(context, vertices[5], vertices[4], vertices[7], vertices[6],
             mapping.back, mapping.material, mapping.transparent, wPixel,
             hPixel);

  RenderQuad(context, vertices[4], vertices[0], vertices[3], vertices[7],
             mapping.right, mapping.material, mapping.transparent, wPixel,
             hPixel);

  RenderQuad(context, vertices[4], vertices[5], vertices[1], vertices[0],
             mapping.bottom, mapping.material, mapping.transparent, wPixel,
             hPixel);

  RenderQuad(context, vertices[3], vertices[2], vertices[6], vertices[7],
             mapping.top, mapping.material, mapping.transparent, wPixel,
             hPixel);
}
...
void RenderSkin(RenderContext &context, int version, Model model) {
  ...
  if (model == Model::Classic) {
    ...
    if (version == 1) {
      ...
      RenderBox(context, 5, 13, 5.03f, left_arm_cover_classic, leftArmPosition);
      RenderBox(context, 5, 13, 5.03f, right_arm_cover_classic,
                rightArmPosition);
      RenderBox(context, 5, 13, 5.01f, left_leg_cover, leftLegPosition);
      RenderBox(context, 5, 13, 5.02f, right_leg_cover, rightLegPosition);
    }
  } else {
    ...
    if (version == 1) {
      RenderBox(context, 9, 13, 5, torso_cover, torsoPosition);
      RenderBox(context, 4, 13, 5.03f, left_arm_cover_slim, leftArmPosition);
      RenderBox(context, 4, 13, 5.03f, right_arm_cover_slim, rightArmPosition);
      RenderBox(context, 5, 13, 5.01f, left_leg_cover, leftLegPosition);
      RenderBox(context, 5, 13, 5.02f, right_leg_cover, rightLegPosition);
    }
  }
...
RenderContext::RenderContext(QOpenGLFunctions &GL, QObject *parent)
    : QObject(parent), GL(GL) {
...
void RenderContext::init() {
  ...
  initShader(m_skinShader, m_uniforms, ":/skins/shaders/skin.vert",
             ":/skins/shaders/skin.frag");
  ...
  initShader(m_bgShader, m_bgUniforms, ":/skins/shaders/bg.vert",
             ":/skins/shaders/bg.frag");
  ...
  m_vao->bind();
  {
    ...
    m_skinShader->setAttributeBuffer(0, GL_FLOAT, offsetof(Vertex, m_position),
                                     3, sizeof(Vertex));
    m_skinShader->enableAttributeArray(0);
    m_skinShader->setAttributeBuffer(1, GL_FLOAT, offsetof(Vertex, m_uv), 2,
                                     sizeof(Vertex));
    m_skinShader->enableAttributeArray(1);
    m_skinShader->setAttributeBuffer(2, GL_INT, offsetof(Vertex, m_texture), 1,
                                     sizeof(Vertex));
    m_skinShader->enableAttributeArray(2);
    m_skinShader->setAttributeBuffer(
        3, GL_INT, offsetof(Vertex, m_transparency), 1, sizeof(Vertex));
  ...
  std::array<VertexBg, 4> backQuad = {VertexBg{-1.0, -1.0f},
                                      VertexBg{1.0, -1.0f}, VertexBg{1.0, 1.0f},
                                      VertexBg{-1.0, 1.0f}};
  std::array<GLuint, 6> backQuadElements = {0, 1, 3, 1, 2, 3};

  m_bgvbo->bind();
  m_bgvbo->allocate(backQuad.data(), (int)(backQuad.size() * sizeof(VertexBg)));
  m_bgvbo->release();

  m_bgebo->bind();
  m_bgebo->allocate(backQuadElements.data(),
                    (int)(backQuadElements.size() * sizeof(GLuint)));
  m_bgebo->release();
}
...
bool RenderContext::initShader(QOpenGLShaderProgram *program,
  ...
  if (!program->addShaderFromSourceFile(QOpenGLShader::Vertex,
                                        vertexShaderPath)) {
    qCritical() << "Error compiling vertex shader " << vertexShaderPath << ": "
                << program->log();
    return false;
  }
  ...
  if (!program->addShaderFromSourceFile(QOpenGLShader::Fragment,
                                        fragmentShaderPath)) {
    qCritical() << "Error compiling fragment shader " << fragmentShaderPath
                << ": " << program->log();
    return false;
  }
...
void RenderContext::regenerateGeometry(int skinVersion, Model skinModel) {
  ...
  m_vbo->bind();
  m_vbo->allocate(m_vertexBuffer.data(),
                  (int)(m_vertexBuffer.size() * sizeof(Vertex)));
  m_vbo->release();

  m_ebo->bind();
  m_ebo->allocate(m_elementBuffer.data(),
                  (int)(m_elementBuffer.size() * sizeof(GLuint)));
  m_ebo->release();
}

void Skins::RenderContext::render(QMatrix4x4 viewMatrix, QVector3D baseColor,
                                  QVector3D alternateColor,
                                  float fDevicePixelSize) {
  ...
  /** TODO:
   * - render pure black version of the model (all color except alpha converted
   * to black) to texture
  ...
  m_vao->bind();
  GL.glDrawElements(GL_TRIANGLES, (GLsizei)m_elementBuffer.size(),
