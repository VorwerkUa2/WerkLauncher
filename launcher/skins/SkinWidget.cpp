...
SkinWidget::SkinWidget(QWidget *parent) : QFrame(parent) {
  // TODO: if OpenGL init fails in the Window, this should provide a fallback
  // with no 3D rendering
  m_window = new SkinWindow();
  m_window->setBackgroundColor(
      palette().color(QPalette::Normal, QPalette::Base));
...
void SkinWidget::setAll(Skins::Model model, const QImage &skinImage,
                        const QImage &capeImage) {
...
void SkinWidget::setModel(Skins::Model model) { m_window->setModel(model); }
...
SkinWindow::SkinWindow() : QOpenGLWindow() {
  ...
  setAll(Skins::Model::Classic, QImage(":/skins/textures/placeholder_skin.png"),
         QImage(":/skins/textures/placeholder_cape.png"));
}

SkinWindow::~SkinWindow() noexcept { m_context->deinit(); }
...
// OK LAB color space conversions for decent determination of lighter/darker
// alternate color NOTE: code adapted from
...
QVector3D getContrastColor(QVector3D color) {
  constexpr float contrast = 0.035f;
...
void SkinWindow::paintGL() {
  ...
  m_context->render(m_worldToView, m_backgroundColor, alternateColor,
                    devicePixelRatioF());
}

void SkinWindow::updateMatrix() {
  QMatrix4x4 camera;
  QVector3D cameraPosition(m_distance * sin(m_spinAngle) * cos(m_liftAngle),
...
void SkinWindow::setAll(Skins::Model model, const QImage &skinImage,
                        const QImage &capeImage) {
  m_model = model;
