...
CapesModel::CapesModel(QObject *parent) : QAbstractListModel(parent) {
  ...
  connect(capeCache.get(), &CapeCache::capeReady, this,
          &CapesModel::capeImageUpdated);
}
...
void CapesModel::setAccount(MinecraftAccountPtr account) {
  ...
  beginResetModel();
  {
    ...
    m_account = account;
    auto capeCache = APPLICATION->capeCache();
    m_capes.push_back(Skins::CapeEntry{
        "", tr("Nothing"),
        QImage(":/skins/textures/no_cape.png")
    ...
    if (m_account) {
      for (auto &cape : m_account->accountData()->minecraftProfile.capes) {
        ...
        entry.preview =
            capeCache->getCapeImage(cape.id)
                .copy(1, 1, 10, 16)
                .scaled(64, 64, Qt::AspectRatioMode::KeepAspectRatio);
        entry.uuid = cape.id;
