...
namespace Skins {
SkinEntry::SkinEntry(const QString &name, const QString &path,
                     const QImage &image, const QString &textureID,
...
SkinEntry::SkinEntry(const QString &name, const QString &pathSlim,
                     const QString &pathClassic)
    : name(name) {
  internal = true;
  {
    QFile file(pathSlim);
...
const QImage &SkinData::getListTexture(Model model) const {
  if (preview.isNull()) {
    ...
    auto paintFront = [&](int x, int y, const Skins::TextureMapping &part) {
      auto partTexture =
          texture.copy(part.front.x, part.front.y, part.front.w, part.front.h);
      if (!part.transparent) {
        painter.fillRect(x, y, part.front.w, part.front.w, Qt::black);
      }
      painter.drawImage(
          x, y, partTexture.mirrored(part.front.flipX, part.front.flipY));
    };
    auto paintBack = [&](int x, int y, const Skins::TextureMapping &part) {
      auto partTexture =
          texture.copy(part.back.x, part.back.y, part.back.w, part.back.h);
      if (!part.transparent) {
        painter.fillRect(x, y, part.back.w, part.back.w, Qt::black);
      }
      painter.drawImage(x, y,
                        partTexture.mirrored(part.back.flipX, part.back.flipY));
    };
    paintFront(4, 2, head);
