...
// NOTE always deep copy on windows. the alternatives are too messy.
  ...
  if (!m_followSymlinks && currentSrc.isSymLink()) {
    ...
    QDir currentDir(src);
    for (auto &f :
         currentDir.entryList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot |
...
bool deletePath(QString path) {
  ...
  for (auto &info : allEntries) {
    ...
    // Windows: check for junctions, reparse points and other nasty things of
    // that sort
...
QString AbsolutePath(QString path) { return QFileInfo(path).absolutePath(); }
 ...
 * Normalize path
 *
 * Any paths inside the current folder will be normalized to relative paths (to
 * current) Other paths will be made absolute
 */
...
HRESULT CreateLink(LPCWSTR linkPath, LPCWSTR targetPath, LPCWSTR args) {
  ...
  if (SUCCEEDED(hres)) {
    ...
    if (SUCCEEDED(hres)) {
      hres = persistFile->Save(linkPath, TRUE);
      persistFile->Release();
    }
    link->Release();
  }
...
bool createShortCut(QString location, QString dest, QStringList args,
  ...
  QString linkPath = PathCombine(location, name + ".lnk");
  QString targetPath = QDir::toNativeSeparators(dest);
  QString argsStr;
  for (int i = 0; i < args.count(); i++) {
    argsStr.append("\"" + args[i] + "\" ");
  }
  return SUCCEEDED(CreateLink((LPCWSTR)linkPath.utf16(),
                              (LPCWSTR)targetPath.utf16(),
                              (LPCWSTR)argsStr.utf16()));
#else
