...
void Parser::setArgumentStyle(ArgumentStyle::Enum style) { m_argStyle = style; }
ArgumentStyle::Enum Parser::argumentStyle() { return m_argStyle; }

void Parser::setFlagStyle(FlagStyle::Enum style) { m_flagStyle = style; }
FlagStyle::Enum Parser::flagStyle() { return m_flagStyle; }
    ...
    // we have (a) flag(s)
    {
      ...
      for (int i = 0; i < flags.length(); i++) {
        ...
        /*
           ...
           multiple times") .arg(option->name, optionPrefix));
        */
        if (option->type == otSwitch)
        ...
        else // if (option->type == otOption)
        {
          if (m_argStyle == ArgumentStyle::Space)
            ...
            if (i == flags.length() - 1)
              ...
              throw ParsingError(
                  QString("Flag %4%2 of Argument-requiring Option "
            ...
            throw ParsingError(
                QString("Option %1 reqires an argument. (flag %3%2)")
  ...
  if (!expecting.isEmpty())
    throw ParsingError(
        QString("Was still expecting arguments for %2%1")
            .arg(expecting.join(QString(", ") + optionPrefix), optionPrefix));

  while (positionals.hasNext()) {
    PositionalDef *param = positionals.next();
    if (param->required)
      throw ParsingError(QString("Missing required positional argument '%1'")
                             .arg(param->name));
    else
