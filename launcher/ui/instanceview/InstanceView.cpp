...
void InstanceView::modelReset() { scheduleDelayedItemsLayout(); }
...
void InstanceView::updateGeometries() {
  ...
  auto proxy = static_cast<InstanceProxyModel *>(model());
  QStringList groups = proxy->getGroups();
...
int InstanceView::itemWidth() const { return m_itemWidth; }
...
void InstanceView::mousePressEvent(QMouseEvent *event) {
  ...
  if (index.isValid()) {
    ...
    if (configRect.contains(visualPos)) {
      emit requestConfig(index);
      return;
    }
  }
...
void InstanceView::mouseDoubleClickEvent(QMouseEvent *event) {
  ...
  QPoint geometryPos = event->pos() + offset();
  VisualGroup::HitResults hitresult;
  auto group = categoryAt(geometryPos, hitresult);
  if (group && (hitresult & VisualGroup::HeaderHit) &&
      !(hitresult & VisualGroup::CheckboxHit)) {
    emit renameGroupRequested(group->text);
    return;
  }
...
void InstanceView::dropEvent(QDropEvent *event) {
  ...
  if (mimedata->hasUrls()) {
    ...
    QModelIndex index = indexAt(event->pos());
    if (index.isValid()) {
      emit droppedURLsOnIndex(urls, index);
    } else {
...
bool InstanceView::isDragEventAccepted(QDropEvent *event) { return true; }
...
InstanceView::rowDropPos(const QPoint &pos) {
  ...
  return {group, hitresult};
}

QStyleOptionViewItem InstanceView::viewOptions() const {
