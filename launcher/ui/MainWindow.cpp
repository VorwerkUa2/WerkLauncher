...
MainWindow::MainWindow(QWidget *parent)
...

  {
    ...
    m_searchBar = new QLineEdit(ui->centralWidget);

    m_searchBar->setPlaceholderText(tr("Search instances..."));
    ...
    connect(m_searchBar, &QLineEdit::textChanged, this,
            ...
            &MainWindow::searchTextChanged);

    m_sortBox = new QComboBox(ui->centralWidget);

    m_sortBox->addItem(tr("Sort by Name"), 0);

    m_sortBox->addItem(tr("Sort by Last Played"), 1);

    m_sortBox->addItem(tr("Sort by Version"), 2);

    QString sortMode = APPLICATION->settings()->get("InstSortMode").toString();

    if (sortMode == "LastLaunch")

      m_sortBox->setCurrentIndex(1);

    else if (sortMode == "Version")

      m_sortBox->setCurrentIndex(2);
    ...
    connect(m_sortBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            ...
            this, &MainWindow::sortModeChanged);
    ...
    connect(view, &InstanceView::droppedURLs, this, &MainWindow::droppedURLs,

            Qt::QueuedConnection);

    connect(view, &InstanceView::droppedURLsOnIndex, this,

            &MainWindow::droppedURLsOnIndex, Qt::QueuedConnection);
  ...
  connect(view, &InstanceView::requestLaunch, this, [this](QModelIndex index) {
  ...
  connect(view, &InstanceView::requestConfig, this, [this](QModelIndex index) {
    view->selectionModel()->setCurrentIndex(
  ...
  connect(APPLICATION->instances().get(), &InstanceList::instanceSelectRequest,

          this, &MainWindow::instanceSelectRequest);
  ...
  connect(APPLICATION->accounts().get(), &AccountList::defaultAccountChanged,
          ...
          this, &MainWindow::defaultAccountChanged);

  connect(APPLICATION->accounts().get(), &AccountList::accountChanged, this,

          &MainWindow::defaultAccountChanged);

  connect(APPLICATION->accounts().get(), &AccountList::listChanged, this,

          &MainWindow::repopulateAccountsMenu);
  ...
  if (BuildConfig.UPDATER_ENABLED) {
    ...
    // NOTE: calling the operator like that is an ugly hack to appease ancient
  ...
  if (QSystemTrayIcon::isSystemTrayAvailable()) {
    ...
    m_trayIcon = new QSystemTrayIcon(APPLICATION->getThemedIcon("logo"), this);

    QMenu *trayMenu = new QMenu(this);

    QAction *showHideAction = trayMenu->addAction(tr("Show/Hide MainWindow"));
    ...
    connect(m_trayIcon, &QSystemTrayIcon::activated, this,
            ...
            &MainWindow::handleTrayIconActivation);
  }
...
void MainWindow::showInstanceContextMenu(const QPoint &pos) {
  ...
  if (onInstance) {
    ...
    if (!group.isNull()) {
      ...
      actions.append(actionCreateGroup);
    }
  }
...
void MainWindow::searchTextChanged(const QString &text) {
  ...
  if (proxymodel) {
    ...
    proxymodel->setSearchFilter(text);
  }
}

void MainWindow::sortModeChanged(int index) {
  ...
  if (index == 0) {
    ...
    APPLICATION->settings()->set("InstSortMode", "LastLaunch");

  } else if (index == 2) {

    APPLICATION->settings()->set("InstSortMode", "Version");
  }
...
void MainWindow::updateToolsMenu() {
  ...
  connect(mmcExport, &QAction::triggered, this,

          &MainWindow::on_actionExportInstance_triggered);
  ...
  for (auto profiler : APPLICATION->profilers().values()) {
    ...
    if (!profiler->check(&error)) {
      ...
      QString profilerToolTip = tr("Profiler not setup correctly. Go into "
...
void MainWindow::downloadUpdates(GoUpdate::Status status) {
  ...
  if (!FS::ensureFilePathExists(dlPath)) {
    ...
    CustomMessageBox::selectable(
        ...
        tr("Couldn't create folder for update downloads:\n%1").arg(dlPath),
  ...
  GoUpdate::DownloadTask updateTask(APPLICATION->network(), status, dlPath,
  ...
  if (updateDlg.execWithTask(&updateTask)) {
     ...
     * NOTE: This disables launching instances until the update either succeeds

     * (and this process exits) or the update fails (and the control leaves this
    ...
    UpdateController update(this, APPLICATION->root(),

                            updateTask.updateFilesDir(),
    ...
    CustomMessageBox::selectable(this, tr("Error"), updateTask.failReason(),
...
template <typename T> T non_stupid_abs(T in) {
...
void MainWindow::finalizeInstance(InstancePtr inst) {
  ...
  if (APPLICATION->accounts()->anyAccountIsValid()) {
    ...
    CustomMessageBox::selectable(
        ...
        tr("The launcher cannot download Minecraft or update instances unless "
...
void MainWindow::droppedURLsOnIndex(QList<QUrl> urls, QModelIndex index) {
  ...
  for (auto &url : urls) {
    ...
    if (url.isLocalFile()) {
      ...
      if (fi.suffix().compare("jar", Qt::CaseInsensitive) == 0 ||

          fi.suffix().compare("zip", Qt::CaseInsensitive) == 0) {

        if (!FS::ensureFilePathExists(modsFolder))

          continue;

        QString target = QDir(modsFolder).absoluteFilePath(fi.fileName());
  ...
  if (addedAny) {
    ...
    ToastNotification::show(this, tr("Mods added to %1").arg(inst->name()),
...
void MainWindow::newsButtonClicked() { on_actionMoreNews_triggered(); }
...
void MainWindow::changeEvent(QEvent *event) {
  if (event->type() == QEvent::LanguageChange) {
    ...
    if (isMinimized() &&
        APPLICATION->settings()->get("MinimizeToTray").toBool()) {
...
void MainWindow::instanceActivated(QModelIndex index) {
  ...
  ToastNotification::show(this, tr("Launching %1...").arg(inst->name()),
...
void MainWindow::checkInstancePathForProblems() {
  ...
  if (FS::checkProblemticPathJava(QDir(instanceFolder))) {
    ...
    warning.setText(tr("Your instance folder contains \'!\' and this is known "

                       "to cause Java problems!"));
  ...
  auto tempFolderText = tr("This is a problem: <br/>"
                           ...
                           " - The launcher will likely be deleted without "

                           "warning by the operating system <br/>"

                           " - close the launcher now and extract it to a real "
  ...
  if (pathfoldername.contains("Rar$", Qt::CaseInsensitive)) {
    ...
    warning.setText(tr("Your instance folder contains \'Rar$\' - that means "

                       "you haven't extracted the launcher archive!"));
    ...
    warning.setText(tr("Your instance folder is in a temporary folder: \'%1\'!")
...
void MainWindow::setCatBackground(bool) {
  // Hidden feature
}
