...
protected:
  ...
  // Skins stuff
private slots:
  void onSkinSelectionChanged(const class QItemSelection &selected,
                              const class QItemSelection &deselected);
  void onCapeSelectionChanged(const class QItemSelection &selected,
                              const class QItemSelection &deselected);
...
private:
  bool startSkinEdit();
  bool hasEdits() const { return m_skinEdit != nonstd::nullopt; }
  void revertEdits();
...
private slots:
  void onSelectMicrosoftClicked(bool);
  void onSelectLittleSkinClicked(bool);
  void onLittleSkinLoginClicked(bool);
  void onLittleSkinCancelClicked(bool);
...
private slots:
  ...
  void onNameCheckFinished(QNetworkReply::NetworkError error, QByteArray data,
