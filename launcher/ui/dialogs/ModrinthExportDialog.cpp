...
ModrinthExportDialog::ModrinthExportDialog(InstancePtr instance,
                                           QWidget *parent)
...
void ModrinthExportDialog::updateDialogState() {
  ui->buttonBox->button(QDialogButtonBox::StandardButton::Ok)
      ->setEnabled(
          !ui->name->text().isEmpty() && !ui->version->text().isEmpty() &&
          ui->file->text().endsWith(".mrpack") &&
          (!ui->includeDatapacks->isChecked() ||
           (!ui->datapacksPath->text().isEmpty() &&
            QDir(m_instance->gameRoot() + "/" + ui->datapacksPath->text())
                .exists())));
}

void ModrinthExportDialog::on_fileBrowseButton_clicked() {
  QFileDialog dialog(
      this, tr("Select modpack file"),
...
void ModrinthExportDialog::on_datapackPathBrowse_clicked() {
  QFileDialog dialog(this, tr("Select global datapacks folder"),
                     m_instance->gameRoot());
  dialog.setAcceptMode(QFileDialog::AcceptOpen);
  dialog.setFileMode(QFileDialog::Directory);

  if (dialog.exec()) {
    ui->datapacksPath->setText(
        QDir(m_instance->gameRoot())
            .relativeFilePath(dialog.selectedFiles().at(0)));
  }
...
void ModrinthExportDialog::accept() {
  ...
  MinecraftInstancePtr minecraftInstance =
      std::dynamic_pointer_cast<MinecraftInstance>(m_instance);
    ...
    if (component->isCustom()) {
      CustomMessageBox::selectable(
          ...
          tr("Instance contains a custom component: %1\nThis cannot be "
             "exported to a Modrinth pack; the exported pack may not work "
             "correctly!")
  ...
  settings.gameVersion =
      minecraftInstance->getPackProfile()->getComponentVersion("net.minecraft");
  settings.forgeVersion =
      minecraftInstance->getPackProfile()->getComponentVersion(
          "net.minecraftforge");
  settings.fabricVersion =
      minecraftInstance->getPackProfile()->getComponentVersion(
          "net.fabricmc.fabric-loader");
  settings.quiltVersion =
      minecraftInstance->getPackProfile()->getComponentVersion(
          "org.quiltmc.quilt-loader");
  settings.neoforgeVersion =
      minecraftInstance->getPackProfile()->getComponentVersion("net.neoforged");
  ...
  connect(task, &Task::failed, [this](QString reason) {
    ...
    CustomMessageBox::selectable(parentWidget(), tr("Error"), text,
  ...
  connect(task, &Task::succeeded, [this, task]() {
    ...
    if (warnings.count()) {
      CustomMessageBox::selectable(parentWidget(), tr("Warnings"),
                                   warnings.join('\n'), QMessageBox::Warning)
          ->show();
    }
  });
...
ModrinthExportDialog::~ModrinthExportDialog() { delete ui; }
