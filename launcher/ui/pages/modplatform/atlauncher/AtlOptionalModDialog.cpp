...
AtlOptionalModListModel::AtlOptionalModListModel(
    QWidget *parent, ATLauncher::PackVersion version,
...
QVariant AtlOptionalModListModel::data(const QModelIndex &index,
                                       int role) const {
  auto row = index.row();
...
void AtlOptionalModListModel::toggleMod(ATLauncher::VersionMod mod, int index) {
  ...
  // If there is a warning for the mod, display that first (if we would be
  // enabling the mod)
  if (enable && !mod.warning.isEmpty() &&
      m_version.warnings.contains(mod.warning)) {
    auto message =
        QString("%1<br><br>%2")
            .arg(m_version.warnings[mod.warning],
                 tr("Are you sure that you want to enable this mod?"));

    // fixme: avoid casting here
    auto result =
        QMessageBox::warning((QWidget *)this->parent(), tr("Warning"), message,
...
AtlOptionalModDialog::AtlOptionalModDialog(QWidget *parent,
  ...
  titleBar->setTitle(tr("Optional Mods"));
  // Inject into the grid layout by moving things down
  ui->gridLayout_2->setContentsMargins(0, 0, 0, 0);
  ui->gridLayout_2->setSpacing(0);
  // move treeView from 0,0 to 1,0
  ui->gridLayout_2->removeWidget(ui->treeView);
  ui->gridLayout_2->addWidget(ui->treeView, 1, 0, 1, 4);
  // move buttons from row 1 to row 2
  ui->gridLayout_2->removeWidget(ui->shareCodeButton);
  ui->gridLayout_2->addWidget(ui->shareCodeButton, 2, 0);
  ui->gridLayout_2->removeWidget(ui->selectRecommendedButton);
  ui->gridLayout_2->addWidget(ui->selectRecommendedButton, 2, 1);
  ui->gridLayout_2->removeWidget(ui->clearAllButton);
  ui->gridLayout_2->addWidget(ui->clearAllButton, 2, 2);
  ui->gridLayout_2->removeWidget(ui->installButton);
  ui->gridLayout_2->addWidget(ui->installButton, 2, 3);
  ui->gridLayout_2->addWidget(titleBar, 0, 0, 1, 4);
...
AtlOptionalModDialog::~AtlOptionalModDialog() { delete ui; }
