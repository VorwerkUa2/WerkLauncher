...
void Modrinth::ListModel::searchRequestFinished() {
  ...
  for (auto packRaw : hits) {
    ...
    try {
      if (Json::ensureString(packObj, "client_side", "required") ==
...
void Modrinth::ListModel::requestLogo(const QString &logo, const QUrl &url) {
  ...
  MetaEntryPtr entry = APPLICATION->metacache()->resolveEntry(
      "ModrinthPacks", QString("logos/%1").arg(logo.section(".", 0, 0)));
  auto *job = new NetJob(QString("Modrinth Icon Download %1").arg(logo),
                         APPLICATION->network());
...
void Modrinth::ListModel::getPackDetails(const QString &id) {
  ...
  if (modpack.detailsLoaded != LoadState::Loaded) {
    ...
    netJob->addNetAction(
        Net::Download::makeByteArray(QUrl(detailsUrl), &detailsResponse));
    detailsPtr = netJob;
    detailsPtr->start();
    QObject::connect(netJob, &NetJob::succeeded, this,
                     &ListModel::detailsRequestFinished);
    QObject::connect(netJob, &NetJob::failed, this,
                     &ListModel::detailsRequestFailed);
  }
  ...
  if (modpack.versionsLoaded != LoadState::Loaded) {
    ...
    netJob->addNetAction(
        Net::Download::makeByteArray(QUrl(versionsUrl), &versionsResponse));
    versionsPtr = netJob;
    versionsPtr->start();
    QObject::connect(netJob, &NetJob::succeeded, this,
                     &ListModel::versionsRequestFinished);
    QObject::connect(netJob, &NetJob::failed, this,
                     &ListModel::versionsRequestFailed);
  }
}
...
nonstd::optional<Modrinth::Modpack>
Modrinth::ListModel::getModpackById(const QString &id) {
  auto index = getIndexFromId(id);
  if (!index) {
    return nonstd::nullopt;
  }
...
bool parseDetailsInto(QByteArray &input, Modrinth::Modpack &output) {
  ...
  if (parse_error.error != QJsonParseError::NoError) {
    qWarning() << "Error while parsing pack details response from Modrinth at "
               << parse_error.offset
               << " reason: " << parse_error.errorString();
    qWarning() << input;
    return false;
    ...
    "files": [
      {
        "hashes": {
          ...
          "sha512":
  "d43e148d35d0267b49ed14a7bb4bb5879aa3ebbf5805c2fe125f0f0f19fd84994242bdcd568399c9ff80ecfbe0e975ab632d8c71773bc09d54cfc857f9a6f716"
        },
        "url":
  "https://cdn.modrinth.com/data/WCJmvhgU/versions/2022.1.12/waffles_Modpack-2022.1.12no-hydrogen.mrpack",
...
bool parseVersionsInto(QByteArray &input, Modrinth::Modpack &output) {
  ...
  QJsonDocument doc = QJsonDocument::fromJson(input, &parse_error);
  if (parse_error.error != QJsonParseError::NoError) {
    qWarning() << "Error while parsing pack versions response from Modrinth at "
               << parse_error.offset
               << " reason: " << parse_error.errorString();
    qWarning() << input;
    return false;
    ...
    for (auto obj : versions) {
      ...
      try {
        ...
        if (versionTypeString == "alpha") {
          ...
          qWarning() << "Unknown version type of Modrinth modpack: "
                     << versionTypeString;
