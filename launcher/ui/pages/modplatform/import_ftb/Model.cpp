...
int Model::rowCount(const QModelIndex &parent) const { return modpacks.size(); }
...
#else
QString getFTBAPath() { return FS::PathCombine(QDir::homePath(), ".ftba"); }
#endif
...
QString getFTBAVersionPath(const QString &id) {
  return FS::PathCombine(getFTBAPath(),
                         QString("bin/versions/%1/%1.json").arg(id));
}
...
QString getFTBAInstances() {
  ...
  try {
    ...
    qCritical() << "Could not read FTB App settings file as JSON: "
                << e.cause();
    return QString();
  }
}
...
Reference from an FTB App file, as of 28.05.2022
{
  ...
  "description": "Direwolf's modded legacy started all the way back in 2011.
With 5 modpacks from the FTB Team, dedicated to creating a simple yet unique
form of kitchen-sink modpacks.\n\nThis new iteration of Direwolf's modpack is
packed with hand-selected mods from Direwolf20 himself, the FTB Team have worked
hard in forging a kitchen-sink styled modded Minecraft experience that just
about anyone can play. \n\nFollow along with Direwolf on his YouTube series,
learning with him as you venture out into new modded territory. Improve his
designs as you build up more infrastructure and automate anything and
everything. The world is yours, for you to do anything in.", "mcVersion":
"1.18.2", "jvmArgs": "", "embeddedJre": true, "url": "", "artUrl":
"https://apps.modpacks.ch/modpacks/art/90/1024_1024.png", "width": 1920,
  "height": 1080,
...
void resolveModloader(QString mcVersion, ModLoader &loader) {
  ...
  try {
    ...
    qWarning() << "Could not resolve modloader ID: " << loader.id
               << "File cannot be loaded: " << path;
  ...
  try {
    ...
    qWarning() << "Could not resolve modloader ID: " << loader.id
               << "File cannot be understood: " << path
               << "Error: " << e.cause();
    loader.type = ModLoaderType::Unresolved;
  }
}
...
bool parseModpackJson(const QByteArray &data, Modpack &out) {
  try {
    ...
    out.description = Json::ensureString(
        object, "description",
        QObject::tr("Description is missing in the FTB App instance."));
...
void Model::reload() {
  ...
  if (instancesLocation.isEmpty()) {
    ...
    QDirIterator iter(instancesLocation,
