...
TechnicPage::TechnicPage(NewInstanceDialog *dialog, QWidget *parent)
  ...
  connect(ui->packView->selectionModel(), &QItemSelectionModel::currentChanged,
          this, &TechnicPage::onSelectionChanged);
  connect(ui->versionSelectionBox, &QComboBox::currentTextChanged, this,
          &TechnicPage::onVersionSelectionChanged);
}
...
TechnicPage::~TechnicPage() { delete ui; }

bool TechnicPage::shouldDisplay() const { return true; }
...
void TechnicPage::suggestCurrent() {
  ...
  QObject::connect(netJob, &NetJob::succeeded, this, [this, slug] {
    ...
    current.websiteUrl =
        Json::ensureString(obj, "platformUrl", QString(), "__placeholder__");
    current.author =
        Json::ensureString(obj, "user", QString(), "__placeholder__");
    current.description =
        Json::ensureString(obj, "description", QString(), "__placeholder__");
    current.currentVersion =
        Json::ensureString(obj, "version", QString(), "__placeholder__");
...
void TechnicPage::metadataLoaded() {
  ...
  if (current.websiteUrl.isEmpty())
    ...
    text = "<a href=\"" + current.websiteUrl.toHtmlEscaped() + "\">" +
           name.toHtmlEscaped() + "</a>";
  ...
  if (!current.isSolder) {
    ...
    // For now, until the versions are pulled from the Solder instance, display
    // the current version so we can display something quicker
    ui->versionSelectionBox->addItem(current.currentVersion);

    auto *netJob =
        new NetJob(QString("Technic::SolderMeta(%1)").arg(current.name),
                   APPLICATION->network());
    auto url = QString("%1/modpack/%2").arg(current.url, current.slug);
    netJob->addNetAction(Net::Download::makeByteArray(QUrl(url), &response));

    QObject::connect(netJob, &NetJob::succeeded, this,
                     &TechnicPage::onSolderLoaded);
  ...
  if (!current.isSolder) {
    ...
    dialog->setSuggestedPack(current.name + " " + selectedVersion,
                             new Technic::SolderPackInstallTask(
                                 ...
                                 selectedVersion, current.minecraftVersion));
  }
}
...
void TechnicPage::onSolderLoaded() {
  ...
  if (parse_error.error != QJsonParseError::NoError) {
    qWarning() << "Error while parsing JSON response from Solder at "
               << parse_error.offset
               << " reason: " << parse_error.errorString();
    fallback();
    return;
