...
FilterModel::Sorting FilterModel::getCurrentSorting() { return currentSorting; }
...
int ListModel::columnCount(const QModelIndex &parent) const { return 1; }
...
Modpack ListModel::at(int row) { return modpacks.at(row); }
...
void ListModel::requestLogo(QString file) {
  ...
  job->addNetAction(Net::Download::makeCached(
      QUrl(
          QString(BuildConfig.LEGACY_FTB_CDN_BASE_URL + "static/%1").arg(file)),
      entry));
