...
public:
  ...
  bool chompString(QByteArray &appendHere) {
    ...
    if (!endsWith('"')) {
      qDebug() << "String fragment does not end with \", instead, there is"
               << at(size() - 1);
      return false;
    }
      ...
      if (escape) {
        ...
        case '7': {
          ...
          while ((c = operator[](i)) >= '0' && c <= '7') {
            i++;
            if (i == length() - 1) {
              qDebug() << "Something went bad while parsing an octal escape "
                          "string...";
              return false;
            }
          }
        ...
        case 'x': {
          ...
          while (isxdigit(operator[](i))) {
            ...
            if (i == length() - 1) {
              qDebug()
                  << "Something went bad while parsing a hex escape string...";
              return false;
            }
          }
...
bool POTranslator::isEmpty() const { return !d->loaded; }
