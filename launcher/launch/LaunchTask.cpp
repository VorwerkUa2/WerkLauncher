...
void LaunchTask::init() { m_instance->setRunning(true); }
...
shared_qobject_ptr<LogModel> LaunchTask::getLogModel() {
  if (!m_logModel) {
    ...
    m_logModel->setOverflowMessage(
        tr("MultiMC stopped watching the game log because the log length "
           "surpassed %1 lines.\n"
           "You may have to fix your mods because the game is still logging to "
           "files and"
