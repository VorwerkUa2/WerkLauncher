...
#define MACOS_HINT                                                             \
  ...
  "/Applications or ~/Applications folder. "                                   \
  "This usually fixes the problem and you can move the application elsewhere " \
  "afterwards.\n"                                                              \
  "\n"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext &context,
  ...
  bool isDefault = !category || strcmp(category, "default") == 0;
  ...
  QString out;
  if (isDefault) {
    out = QString("%1 %2 %3\n").arg(buf).arg(levels[type]).arg(msg);
  } else {
    out = QString("%1 %2 %3: %4\n")
              .arg(buf)
              .arg(levels[type])
...
QString getIdealPlatform(QString currentPlatform) {
  ...
  case Sys::KernelType::Windows: {
    // FIXME: 5.15.2 is not stable on Windows, due to a large number of
    // completely unpredictable and hard to reproduce issues
    break;
    /*
...
Application::Application(int &argc, char **argv) : QApplication(argc, argv) {
  ...
  if (AttachConsole(ATTACH_PARENT_PROCESS)) {
    ...
    DWORD written;
    const char *endline = "\n";
    WriteConsole(out, endline, (DWORD)strlen(endline), &written, NULL);
  ...
  // High-level workarounds for Qt 5.15.2 networking issues on Windows
  qputenv("QT_BEARER_POLL_TIMEOUT",
          "-1"); // Disable problematic bearer management
  qRegisterMetaType<MinecraftAccountPtr>("MinecraftAccountPtr");
  ...
  QHash<QString, QVariant> args;
  {
    ...
    parser.addDocumentation(
        "dir", "Use the supplied folder as application root instead of the "
               "binary location (use '.' for current)");
    // --launch
    ...
    parser.addDocumentation(
        "server",
        "Join the specified server on launch (only valid in combination with "
        "--launch, mutually exclusive with --world)");
    // --world
    parser.addOption("world");
    parser.addShortOpt("world", 'w');
    parser.addDocumentation(
        "world",
        "Join the singleplayer world with the specified folder name on launch "
        "(only valid in combination with --launch, mutually exclusive with "
        "--server, only works with Minecraft 23w14a and later)");
    // --profile
    ...
    parser.addDocumentation(
        "offline", "Launch offline (only valid in combination with --launch)");
    // --name
    parser.addOption("name");
    parser.addShortOpt("name", 'n');
    parser.addDocumentation(
        "name", "When launching offline, use specified name (only makes sense "
                "in combination with --launch and --offline)");
    // --alive
    ...
    if (args["version"].toBool()) {
      std::cout << "Version "
                << BuildConfig.printableVersionString().toStdString()
                << std::endl;
  ...
  if (!dirParam.isEmpty()) {
    // the dir param. it makes multimc data path point to whatever the user
  ...
  if (!FS::ensureFolderPathExists(dataPath)) {
    showFatalErrorMessage(
        ...
        QString("The launcher data folder could not be created.\n"
                ...
                MACOS_HINT
#endif
                "Make sure you have the right permissions to the launcher data "
  ...
  if (m_instanceIdToLaunch.isEmpty()) {
    if (!m_serverToJoin.isEmpty()) {
      std::cerr << "--server can only be used in combination with --launch!"
                << std::endl;
      m_status = Application::Failed;
      return;
    }
    ...
    if (!m_worldToJoin.isEmpty()) {
      std::cerr << "--world can only be used in combination with --launch!"
                << std::endl;
      m_status = Application::Failed;
      return;
    }
    ...
    if (!m_profileToUse.isEmpty()) {
      std::cerr << "--account can only be used in combination with --launch!"
                << std::endl;
      m_status = Application::Failed;
      return;
    }
    ...
    if (m_offline) {
      std::cerr << "--offline can only be used in combination with --launch!"
                << std::endl;
      m_status = Application::Failed;
      return;
    }
    ...
    if (!m_offlineName.isEmpty()) {
      std::cerr << "--offlineName can only be used in combination with "
                   "--launch and --offline!"
                << std::endl;
      m_status = Application::Failed;
      return;
...
#if defined(Q_OS_MAC)
  // move user data to new location if on macOS and it still exists in
  // Contents/MacOS
  QDir fi(applicationDirPath());
  QString originalData = fi.absolutePath();
  // if the config file exists in Contents/MacOS, then user data is still there
  // and needs to moved
  if (QFileInfo::exists(
          FS::PathCombine(originalData, BuildConfig.LAUNCHER_CONFIGFILE))) {
    if (!QFileInfo::exists(FS::PathCombine(originalData, "dontmovemacdata"))) {
      ...
      askMoveDialogue = QMessageBox::question(
          ...
          "Would you like to move application data to a new data location? It "
          "will improve the launcher's performance, but if you switch to older "
          "versions it will look like instances have disappeared. If you "
          "select no, you can migrate later in settings. You should select yes "
          "unless you're commonly switching between different versions (eg. "
          "develop and stable).",
          QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
      if (askMoveDialogue == QMessageBox::Yes) {
        qDebug() << "On macOS and found config file in old location, moving "
                    "user data...";
        QDir dir;
  ...
  /*
   * Establish the mechanism for communication with an already running MultiMC
   * that uses the same data path. If there is one, tell it what the user
   * actually wanted to do and exit. We want to initialize this before logging
   * to avoid messing with the log of a potential already running copy.
   */
  auto appID = ApplicationId::fromPathAndVersion(
      QDir::currentPath(), BuildConfig.printableVersionString());
  {
    // FIXME: you can run the same binaries with multiple data dirs and they
    // won't clash. This could cause issues for updates.
  ...
  // init the logger
  {
    ...
    if (!logFile->open(QIODevice::WriteOnly | QIODevice::Text |
      ...
      showFatalErrorMessage(
          ...
          QString("The launcher couldn't create a log file - the data folder "
                  "is not writable.\n"
                  "\n"
#if defined(Q_OS_MAC)
                  MACOS_HINT
    ...
    qDebug() << "SSL Library Build Version:"
  ...
  // Set up paths
  {
    ...
    QDir foo(FS::PathCombine(binPath, "../.."));
    m_rootPath = foo.absolutePath();
    // on macOS, touch the root to force Finder to reload the .app metadata (and
    // fix any icon change issues)
  ...
  {
    m_settings.reset(
        new INISettingsObject(BuildConfig.LAUNCHER_CONFIGFILE, this));
    // Updates
    ...
    // Global Skin Sync (for offline/cracked accounts)
    m_settings->registerSetting("GlobalSkinSyncUrl", QString());
    m_settings->registerSetting("GlobalSkinSyncUser", QString());
    m_settings->registerSetting("GlobalSkinSyncPass", QString());
  ...
  if (BuildConfig.UPDATER_ENABLED) {
    ...
    qDebug() << "Initializing updater with platform: " << platform << " -- "
             << channelUrl;
    m_updateChecker.reset(
        new UpdateChecker(m_network, channelUrl, BuildConfig.VERSION_BUILD));
  ...
  // Icon themes
  {
    // TODO: icon themes and instance icons do not mesh well together. Rearrange
    // and fix discrepancies! set icon theme search path!
  ...
  // Initialize widget themes
  {
    auto insertTheme = [this](ITheme *theme) {
      m_themes.insert(
          std::make_pair(theme->id(), std::unique_ptr<ITheme>(theme)));
    };
  ...
  // initialize and load all instances
  {
    ...
    // instance path: check for problems with '!' in instance path and warn the
    // user in the log and remember that we have to show him a dialog when the
    // gui starts (if it does so)
    QString instDir = InstDirSetting->get().toString();
    qDebug() << "Instance path              : " << instDir;
    if (FS::checkProblemticPathJava(QDir(instDir))) {
      qWarning() << "Your instance path contains \'!\' and this is known to "
                    "cause java problems!";
    }
    m_instances.reset(new InstanceList(m_settings, instDir, this));
    connect(InstDirSetting.get(), &Setting::SettingChanged, m_instances.get(),
  ...
  m_profilers.insert("jprofiler", std::shared_ptr<BaseProfilerFactory>(
                                      new JProfilerFactory()));
  m_profilers.insert("jvisualvm", std::shared_ptr<BaseProfilerFactory>(
                                      new JVisualVMFactory()));
...

  {
    ...
    QString themeName = settings()->get("ApplicationTheme").toString();
    if (m_themes.find(themeName) == m_themes.end()) {
      themeName = "dark";
      settings()->set("ApplicationTheme", themeName);
    }
    setApplicationTheme(themeName, true);
...
void Application::performMainStartupAction() {
  ...
  if (!m_instanceIdToLaunch.isEmpty()) {
    ...
    if (inst) {
      ...
      if (!m_serverToJoin.isEmpty()) {
        if (m_serverToJoin.contains(':') || m_serverToJoin.contains('.')) {
          serverOrWorldToJoin.reset(new QuickPlayTarget(
              QuickPlayTarget::parseMultiplayer(m_serverToJoin)));
          qDebug() << "   Launching with server" << m_serverToJoin;
        } else {
          qWarning() << "   Invalid server address ignored:" << m_serverToJoin;
        }
      }

      if (!m_worldToJoin.isEmpty()) {
        serverOrWorldToJoin.reset(new QuickPlayTarget(
            QuickPlayTarget::parseSingleplayer(m_worldToJoin)));
        qDebug() << "   Launching with world" << m_worldToJoin;
      }

      if (!m_profileToUse.isEmpty()) {
        int dummyRow;
        if (!accounts()->getAccountByProfileName(m_profileToUse, accountToUse,
                                                 dummyRow)) {
          return;
        }
      ...
      launch(inst, !offline, nullptr, serverOrWorldToJoin, accountToUse,
             m_offlineName);
      return;
    }
  }
...
void Application::messageReceived(const QByteArray &message) {
  ...
  if (command == "activate") {
    ...
    if (!id.isEmpty()) {
      ...
      if (!instance) {
        qWarning() << "Launch command requires an valid instance ID. " << id
                   << "resolves to nothing.";
        return;
      }
    } else {
    ...
    if (!server.isEmpty()) {
      quickPlayTarget = std::make_shared<QuickPlayTarget>(
          QuickPlayTarget::parseMultiplayer(server));
    } else if (!world.isEmpty()) {
      quickPlayTarget = std::make_shared<QuickPlayTarget>(
          QuickPlayTarget::parseSingleplayer(world));
    }

    MinecraftAccountPtr accountObject;
    if (!profile.isEmpty()) {
      int dummyRow;
      if (!accounts()->getAccountByProfileName(profile, accountObject,
                                               dummyRow)) {
        qWarning()
            << "Launch command requires the specified profile to be valid. "
            << profile << "does not resolve to any account.";
        return;
      }
    }

    launch(instance, !offline, nullptr, quickPlayTarget, accountObject,
           offlineName);
  } else {
...
void Application::setApplicationTheme(const QString &name, bool initial) {
  qDebug() << "Request to set theme:" << name << "initial:" << initial;
  ...
  if (themeIter != m_themes.end()) {
    ...
    qDebug() << "Delegating apply() to theme object.";
    theme->apply(initial);
    m_currentTheme = name;
    m_isApplyingTheme = false;
    qDebug() << "Theme application finished.";
  } else {
...
bool Application::openJsonEditor(const QString &filename) {
  ...
  if (m_settings->get("JsonEditor").toString().isEmpty()) {
    ...
    // return
    // DesktopServices::openFile(m_settings->get("JsonEditor").toString(),
    // file);
...
bool Application::launch(InstancePtr instance, bool online,
  ...
  if (m_updateRunning) {
    qDebug() << "Cannot launch instances while an update is running. Please "
                "try again when updates are completed.";
...
void Application::subRunningInstance() {
  if (m_runningInstances == 0) {
    qCritical() << "Something went really wrong and we now have less than 0 "
                   "running instances... WTF";
    return;
  }
  m_runningInstances--;
  if (m_runningInstances == 0) {
...
bool Application::updatesAreAllowed() { return m_runningInstances == 0; }

void Application::updateIsRunning(bool running) { m_updateRunning = running; }
...
QString Application::msaClientId() const { return BuildConfig.MSA_CLIENT_ID; }
