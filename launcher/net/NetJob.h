...
public:
  ...
  NetAction::Ptr operator[](int index) { return downloads[index]; }
  const NetAction::Ptr at(const int index) { return downloads.at(index); }
  NetAction::Ptr first() {
    if (downloads.size())
      return downloads[0];
...
private:
  ...
  bool m_startingMoreParts = false;
  bool m_needsMoreParts = false;
};
