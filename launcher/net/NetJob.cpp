...
void NetJob::partProgress(int index, qint64 bytesReceived, qint64 bytesTotal) {
  ...
  qint64 inprogress =
      (bytesTotalAll == 0) ? 0 : (bytesAll * 1000) / bytesTotalAll;
  auto current = done * 1000 + doing * inprogress;
  auto current_total = all * 1000;
  // HACK: make sure it never jumps backwards.
  // FAIL: This breaks if the size is not known (or is it something else?) and
  // jumps to 1000, so if it is 1000 reset it to inprogress
...
void NetJob::startMoreParts() {
  if (!isRunning()) {
    // this actually makes sense. You can put running downloads into a NetJob
    // and then not start it until much later.
    return;
  }

  // Re-entrancy guard: cache-hit downloads emit succeeded synchronously
  // during part->start(), which calls partSucceeded() -> startMoreParts()
  // recursively. Without this guard, the tracking sets get corrupted.
  if (m_startingMoreParts) {
    m_needsMoreParts = true;
    return;
  }
  m_startingMoreParts = true;

  do {
    m_needsMoreParts = false;
...
bool NetJob::abort() {
  ...
  // Download::abort() disconnects signals, so NetJob never gets partAborted.
  // We must clean up m_doing ourselves and emit the abort signal.
  m_failed.unite(m_doing);
  m_doing.clear();
  m_aborted = true;
  emitAborted();
  return fullyAborted;
