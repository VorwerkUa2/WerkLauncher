...
public:
  ...
  virtual ~NetAction() { abort(); }

  bool isRunning() const { return m_status == Job_InProgress; }
  bool isFinished() const { return m_status >= Job_Finished; }
  bool wasSuccessful() const {
    return m_status == Job_Finished || m_status == Job_Failed_Proceed;
  }

  qint64 totalProgress() const { return m_total_progress; }
  qint64 currentProgress() const { return m_progress; }
  virtual bool abort() {
    if (m_reply) {
