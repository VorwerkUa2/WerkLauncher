...
Download::Download() : NetAction() { m_status = Job_NotStarted; }
...
void Download::addValidator(Validator *v) { m_sink->addValidator(v); }
...
void Download::startImpl() {
  ...
  case Job_InProgress:
    ...
    break;
  case Job_Failed_Proceed: // this is meaningless in this context. We do need a
                           // sink.
  case Job_NotStarted:
  case Job_Failed:
...
bool Download::handleRedirect() {
  ...
  if (!redirect.isValid()) {
    ...
    // there is a Location header, but it's not correct. we need to apply some
    // workarounds...
    ...
    /*
     * Next, make sure the URL is parsed in tolerant mode. Qt doesn't parse the
     * location header in tolerant mode, which causes issues.
...
void Download::downloadFinished() {
  ...
  m_status = m_sink->finalize(*m_reply);
...
void Download::downloadReadyRead() {
  ...
  if (m_status == Job_InProgress) {
    ...
    // qDebug() << "Download" << m_url.toString() << "gained" << data.size() <<
    // "bytes";
  } else {
    qCritical() << "Cannot write to " << m_target_path << ", illegal status"
                << m_status;
  }
}
...
bool Net::Download::canAbort() { return true; }
