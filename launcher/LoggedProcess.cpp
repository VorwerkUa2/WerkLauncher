...
void LoggedProcess::on_exit(int exit_code, QProcess::ExitStatus status) {
  ...
  if (!m_is_aborting) {
    if (status == QProcess::NormalExit) {
      ...
      emit log({tr("Process exited with exit code %1 (0x%2).")
                    .arg(exit_code)
                    .arg(exit_code, 0, 16)},
      ...
      if (exit_code == -1)
        ...
        emit log({tr("Process crashed with exit code %1 (0x%2).")
                      .arg(exit_code)
                      .arg(exit_code, 0, 16)},
                 MessageLevel::Launcher);
    ...
    if ((exit_code < -1 || exit_code > 1) && (exit_code != 255)) {
      ...
      bool hasNameOrDescription = Sys::lookupSystemStatusCode(
          u_exit_code, statusName, statusDescription);
      if (hasNameOrDescription) {
        emit log({tr("Below is an analysis of the exit code. THIS MAY BE "
                     "INCORRECT AND SHOULD BE TAKEN WITH A GRAIN OF SALT!")},
                 MessageLevel::Launcher);

        if (!statusName.empty()) {
          emit log({tr("System exit code name: %1")
                        .arg(QString::fromStdString(statusName))},
                   MessageLevel::Launcher);
        }

        if (!statusDescription.empty()) {
          emit log({tr("System exit code description: %1")
                        .arg(QString::fromStdString(statusDescription))},
                   MessageLevel::Launcher);
        }
      }
    }

    emit log({tr("Please note that usually neither the exit code, nor its "
                 "description are enough to diagnose issues!")},
             MessageLevel::Launcher);
    emit log({tr("Always upload the entire log and not just the exit code.")},
             MessageLevel::Launcher);
  } else {
...
int LoggedProcess::exitCode() const { return m_exit_code; }
...
LoggedProcess::State LoggedProcess::state() const { return m_state; }
...
void LoggedProcess::on_stateChange(QProcess::ProcessState state) {
  ...
  case QProcess::Starting: {
    if (m_state != LoggedProcess::NotRunning) {
      qWarning() << "Wrong state change for process from state" << m_state
                 << "to" << (int)LoggedProcess::Starting;
    }
...
qint64 LoggedProcess::processId() const { return QProcess::processId(); }
