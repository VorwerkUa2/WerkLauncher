...
bool parseVersionInfo(const QByteArray &data, VersionFileList &list,
  ...
  qCDebug(L_UPDATER) << data;
  qCDebug(L_UPDATER) << "Loading version info from JSON.";
  ...
  for (QJsonValue fileValue : filesArray) {
    ...
    qCDebug(L_UPDATER) << "File" << file.path << "with perms" << file.mode;
    ...
    for (QJsonValue val : sourceArray) {
      ...
      if (type == "http") {
        ...
        qCWarning(L_UPDATER) << "Unknown source type" << type << "ignored.";
      }
    }

    qCDebug(L_UPDATER) << "Loaded info for" << file.path;
...
bool processFileLists(const VersionFileList &currentVersion,
  ...
  // First, if we've loaded the current version's file list, we need to iterate
  // through it and delete anything in the current one version's list that isn't
  // in the new version's list.
  for (VersionFileEntry entry : currentVersion) {
    QFileInfo toDelete(FS::PathCombine(rootPath, entry.path));
    if (!toDelete.exists()) {
      qCCritical(L_UPDATER) << "Expected file " << toDelete.absoluteFilePath()
                            << " doesn't exist!";
    }
    bool keep = false;
    ...
    for (VersionFileEntry newEntry : newVersion) {
      if (newEntry.path == entry.path) {
        qCDebug(L_UPDATER) << "Not deleting" << entry.path
  ...
  // Next, check each file in MultiMC's folder and see if we need to update
  // them.
  for (VersionFileEntry entry : newVersion) {
    // TODO: Let's not MD5sum a ton of files on the GUI thread. We should
    // probably find a way to do this in the background.
    QString fileMD5;
    ...
    if (!entryFile.exists()) {
      ...
      if (!entryFile.open(QFile::ReadOnly)) {
        qCCritical(L_UPDATER)
            << "File " << realEntryPath << " cannot be opened for reading.";
        pass = false;
      }
      if (!pass) {
        ops.clear();
    ...
    if (!needs_upgrade) {
      ...
      if ((fileMD5 != entry.md5)) {
        qCDebug(L_UPDATER) << "MD5Sum does not match!";
        qCDebug(L_UPDATER) << "Expected:'" << entry.md5 << "'";
        qCDebug(L_UPDATER) << "Got:     '" << fileMD5 << "'";
        needs_upgrade = true;
      }
    }
    ...
    if (!needs_upgrade) {
      qCDebug(L_UPDATER) << "File" << realEntryPath
                         << " does not need updating.";
      continue;
    }

    // yep. this file actually needs an upgrade. PROCEED.
    qCDebug(L_UPDATER) << "Found file" << realEntryPath
                       << " that needs updating.";

    // Go through the sources list and find one to use.
    // TODO: Make a NetAction that takes a source list and tries each of them
    // until one works. For now, we'll just use the first http one.
    for (FileSource source : entry.sources) {
      if (source.type != "http")
        continue;

      qCDebug(L_UPDATER) << "Will download" << entry.path << "from"
                         << source.url;

      // Download it to updatedir/<filepath>-<md5> where filepath is the file's
      // path with slashes replaced by underscores.
      QString dlPath =
          FS::PathCombine(tempPath, QString(entry.path).replace("/", "_"));
