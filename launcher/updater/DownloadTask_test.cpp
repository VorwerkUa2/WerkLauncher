...
// Robust helper to find test files in the build directory
QByteArray getTestFileData(const QString &relativePathComponent) {
  QString path = FS::PathCombine(QCoreApplication::applicationDirPath(),
                                 relativePathComponent);
  QFile f(path);
  if (!f.open(QFile::ReadOnly)) {
    qWarning() << "Failed to open test file:" << path;
    return QByteArray();
  }
  return f.readAll();
}
...
FileSourceList encodeBaseFile(const char *suffix) {
  ...
  return FileSourceList({FileSource("http", localFile.toString())});
}

QByteArray processTestData(QByteArray data) {
  auto base = QCoreApplication::applicationDirPath();
  auto baseUrl = QUrl::fromLocalFile(base + "/data").toString();
  return data.replace("@TEST_DATA_URL@", baseUrl.toUtf8());
}
...
private slots:
  ...
  void test_parseVersionInfo_data() {
    ...
    QTest::newRow("one")
        << processTestData(getTestFileData("data/1.json"))
        << (VersionFileList()
            << VersionFileEntry{"fileOne", 493,
                                encodeBaseFile("/data/fileOneA"),
                                "9eb84090956c484e32cb6c08455a667b"}
            << VersionFileEntry{"fileTwo", 644, encodeBaseFile("/data/fileTwo"),
                                "38f94f54fa3eb72b0ea836538c10b043"}
            << VersionFileEntry{"fileThree", 750,
                                encodeBaseFile("/data/fileThree"),
                                "f12df554b21e320be6471d7154130e70"})
        << QString() << true;
    QTest::newRow("two")
        << processTestData(getTestFileData("data/2.json"))
        << (VersionFileList()
            << VersionFileEntry{"fileOne", 493,
  ...
  void test_parseVersionInfo() {
    ...
    if (outList != list) {
      qDebug() << "Parsed list:" << outList;
      qDebug() << "Expected list:" << list;
    }
    QCOMPARE(outRet, ret);
  ...
  void test_processFileLists_data() {
    ...
    QTest::newRow("test 1")
        << tempFolder
        << (VersionFileList()
            << VersionFileEntry{"data/fileOne", 493,
                                FileSourceList() << FileSource(
                                    "http", "http://host/path/fileOne-1"),
                                "9eb84090956c484e32cb6c08455a667b"}
            << VersionFileEntry{"data/fileTwo", 644,
                                FileSourceList() << FileSource(
                                    "http", "http://host/path/fileTwo-1"),
                                "38f94f54fa3eb72b0ea836538c10b043"}
            << VersionFileEntry{"data/fileThree", 420,
                                FileSourceList() << FileSource(
  ...
  void test_processFileLists() {
    ...
    // Keep tempFolder directory alive!
    ...
    processFileLists(currentVersion, newVersion,
                     QCoreApplication::applicationDirPath(), tempFolder,
                     new NetJob("Dummy", network), operations);

    if (operations != expectedOperations) {
      qDebug() << "Operations:" << operations;
      qDebug() << "Expected:" << expectedOperations;
    }
