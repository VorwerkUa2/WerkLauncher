...
DownloadTask::DownloadTask(shared_qobject_ptr<QNetworkAccessManager> network,
                           Status status, QString target, QObject *parent)
...
void DownloadTask::executeTask() { loadVersionInfo(); }
...
void DownloadTask::loadVersionInfo() {
  ...
  qCDebug(L_UPDATER) << m_status.newRepoUrl << " turns into " << newIndexUrl;

  netJob->addNetAction(
      m_newVersionFileListDownload =
          Net::Download::makeByteArray(newIndexUrl, &newVersionFileListData));

  // If we have a current version URL, get that one too.
  if (!m_status.currentRepoUrl.isEmpty()) {
    QUrl cIndexUrl =
        QUrl(m_status.currentRepoUrl)
            .resolved(QString::number(m_status.currentVersionId) + ".json");
    netJob->addNetAction(m_currentVersionFileListDownload =
                             Net::Download::makeByteArray(
                                 cIndexUrl, &currentVersionFileListData));
    qCDebug(L_UPDATER) << m_status.currentRepoUrl << " turns into "
                       << cIndexUrl;
  }
...
void DownloadTask::vinfoDownloadFailed() {
  ...
  qCCritical(L_UPDATER) << "Failed to download version info files.";
...
void DownloadTask::processDownloadedVersionInfo() {
  ...
  qCDebug(L_UPDATER) << "Reading file list for new version...";
  QString error;
  if (!parseVersionInfo(newVersionFileListData, m_newVersionFileList, error)) {
    qCCritical(L_UPDATER) << error;
    emitFailed(error);
    return;
  }
  ...
  if (m_currentVersionFileListDownload &&
    ...
    qCDebug(L_UPDATER) << "Reading file list for current version...";
    // if this fails, it's not a complete loss.
    QString error;
    if (!parseVersionInfo(currentVersionFileListData, m_currentVersionFileList,
                          error)) {
      qCDebug(L_UPDATER) << error << "This is not a fatal error.";
    }
  }
  ...
  setStatus(
      tr("Processing file lists - figuring out how to install the update..."));
  ...
  if (!processFileLists(m_currentVersionFileList, m_newVersionFileList,
                        m_status.rootPath, m_updateFilesDir.path(), netJob,
                        m_operations)) {
  ...
  if (netJob->size() == 1) // Translation issues... see
  ...
  qCDebug(L_UPDATER) << "Begin downloading update files to"
                     << m_updateFilesDir.path();
  m_filesNetJob = netJob;
  m_filesNetJob->start();
}

void DownloadTask::fileDownloadFinished() { emitSucceeded(); }

void DownloadTask::fileDownloadFailed(QString reason) {
  qCCritical(L_UPDATER) << "Failed to download update files:" << reason;
...
QString DownloadTask::updateFilesDir() { return m_updateFilesDir.path(); }

OperationList DownloadTask::operations() { return m_operations; }
