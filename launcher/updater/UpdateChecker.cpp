...
UpdateChecker::UpdateChecker(shared_qobject_ptr<QNetworkAccessManager> nam,
                             QString channelUrl, int currentBuild) {
  m_network = nam;
...
bool UpdateChecker::hasChannels() const { return !m_channels.isEmpty(); }

void UpdateChecker::checkForUpdate(bool notifyNoUpdate) {
  qCDebug(L_UPDATER) << "Checking for updates.";
  QString updateChannel = "develop";

  // If the channel list hasn't loaded yet, load it and defer checking for
  // updates until later.
  if (!m_chanListLoaded) {
    qDebug() << "Channel list isn't loaded yet. Loading channel list and "
                "deferring update check.";
  ...
  if (m_updateChecking) {
    qCDebug(L_UPDATER)
        << "Ignoring update check request. Already checking for updates.";
    return;
  }

  // Find the desired channel within the channel list and get its repo URL. If
  // if cannot be found, error.
  QString developUrl;
  m_newRepoUrl = "";
  for (ChannelListEntry entry : m_channels) {
    qCDebug(L_UPDATER) << "channelEntry = " << entry.id;
    if (entry.id == "develop") {
      developUrl = entry.url;
    }
    if (entry.id == updateChannel) {
      m_newRepoUrl = entry.url;
      qCDebug(L_UPDATER) << "is intended update channel: " << entry.id;
    }
    if (entry.id == m_currentChannel) {
      m_currentRepoUrl = entry.url;
      qCDebug(L_UPDATER) << "is current update channel: " << entry.id;
    }
  }

  qCDebug(L_UPDATER) << "m_repoUrl = " << m_newRepoUrl;

  if (m_newRepoUrl.isEmpty()) {
    qWarning() << "m_repoUrl was empty. defaulting to 'develop': "
               << developUrl;
  ...
  if (m_newRepoUrl.isEmpty()) {
    qCritical() << "failed to select any update repository for: "
                << updateChannel;
  ...
  connect(indexJob.get(), &NetJob::succeeded, this,
          [this, notifyNoUpdate]() { updateCheckFinished(notifyNoUpdate); });
  connect(indexJob.get(), &NetJob::failed, this,
          &UpdateChecker::updateCheckFailed);
  indexJob->start();
}

void UpdateChecker::updateCheckFinished(bool notifyNoUpdate) {
  qCDebug(L_UPDATER)
      << "Finished downloading repo index. Checking for new versions.";
  ...
  if (apiVersion != API_VERSION || !success) {
    qCritical()
        << "Failed to check for updates. API version mismatch. We're using"
        << API_VERSION << "server has" << apiVersion;
    m_updateChecking = false;
    return;
  }

  qCDebug(L_UPDATER) << "Processing repository version list.";
  ...
  // We've got the version with the greatest ID number. Now compare it to our
  // current build number and update if they're different.
  int newBuildNumber = newestVersion.value("Id").toVariant().toInt();
  if (newBuildNumber != m_currentBuild) {
    qCDebug(L_UPDATER) << "Found newer version with ID" << newBuildNumber;
    // Update!
...
void UpdateChecker::updateChanList(bool notifyNoUpdate) {
  qCDebug(L_UPDATER) << "Loading the channel list.";

  if (m_chanListLoading) {
    qCDebug(L_UPDATER)
        << "Ignoring channel list update request. Already grabbing "
        << "channel list.";
    return;
  }
  ...
  chanListJob->addNetAction(
      Net::Download::makeByteArray(QUrl(m_channelUrl), &chanlistData));
  connect(
      chanListJob.get(), &NetJob::succeeded, this,
      [this, notifyNoUpdate]() { chanListDownloadFinished(notifyNoUpdate); });
  connect(chanListJob.get(), &NetJob::failed, this,
          &UpdateChecker::chanListDownloadFailed);
  chanListJob->start();
}

void UpdateChecker::chanListDownloadFinished(bool notifyNoUpdate) {
  qCDebug(L_UPDATER) << "UpdateChecker::chanListDownloadFinished"
                     << chanlistData.size() << "bytes downloaded.";
  chanListJob.reset();
  ...
  if (jsonError.error != QJsonParseError::NoError) {
    ...
    qCritical() << "Failed to parse channel list JSON:"
  ...
  bool success = false;
  int formatVersion =
      object.value("format_version").toVariant().toInt(&success);
  if (formatVersion != CHANLIST_FORMAT || !success) {
    qCCritical(L_UPDATER)
        << "Failed to check for updates. Channel list format version "
           "mismatch. We're using"
  ...
  for (QJsonValue chanVal : channelArray) {
    ...
    if (entry.id.isEmpty() || entry.name.isEmpty() || entry.url.isEmpty()) {
      qCritical()
          << "Channel list entry with empty ID, name, or URL. Skipping.";
      continue;
    }
  ...
  qCDebug(L_UPDATER) << "Successfully loaded UpdateChecker channel list.";
