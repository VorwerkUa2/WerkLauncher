...
bool JavaCommon::checkJVMArgs(QString jvmargs, QWidget *parent) {
  if (jvmargs.contains("-XX:PermSize=") ||
    ...
    auto warnStr =
        QObject::tr("You tried to manually set a JVM memory option (using "
                    ...
                    "There are dedicated boxes for these in the settings (Java "
                    "tab, in the Memory group at the top).\n"
                    "This message will be displayed until you remove them from "
                    "the JVM arguments.");
    CustomMessageBox::selectable(parent, QObject::tr("JVM arguments warning"),
  ...
  if (jvmargs.contains(QRegularExpression("-version:.*"))) {
    auto warnStr = QObject::tr(
        "You tried to pass required java version argument to the JVM (using "
        "\"-version=xxx\"). This is not safe and will not be allowed.\n"
        "This message will be displayed until you remove this from the JVM "
        "arguments.");
    CustomMessageBox::selectable(parent, QObject::tr("JVM arguments warning"),
                                 warnStr, QMessageBox::Warning)
        ->exec();
...
void JavaCommon::javaWasOk(QWidget *parent, JavaCheckResult result) {
  ...
  text +=
      QObject::tr(
          ...
          "reported: %3<br />")
          .arg(result.architecture.serialize(), result.javaVersion.toString(),
               result.javaVendor);
  if (result.errorLog.size()) {
    auto htmlError = result.errorLog;
    htmlError.replace('\n', "<br />");
    text += QObject::tr("<br />Warnings:<br /><font color=\"orange\">%1</font>")
                .arg(htmlError);
  }
...
void JavaCommon::javaBinaryWasBad(QWidget *parent, JavaCheckResult result) {
  QString text;
  text += QObject::tr("The specified java binary didn't work.<br />You should "
